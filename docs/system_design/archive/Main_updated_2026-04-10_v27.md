...
- **Incremental update note:** 本版以 `Main_updated_2026-04-10_v26.md` 为 base，仅增量吸收 `P3.3.6 closeout` 与 `BR / UI v0.2.7` runtime-baseline update；未被本轮触发的既有主线程内容保持不动。
...
15. **P3.3.6 — Local-Serving Compatibility Contract / Shadow-Mode Entry Round（Closed）**
   - 本轮完成的是 **Phase 1 / Compatibility Contract v1 + Shadow-Entry Prep**，不是 runtime owner shift completed，也不是 ReviewPage local-serving cutover
   - `local_serving_candidate_contract_v1` 已按 very narrow landing 落地：`local_due_queue_candidate` / `local_generated_review_session_candidate` 只进入 candidate / compatibility / shadow 层，不得写成 current ReviewPage truth
   - `review_group_compatibility_posture_v1` 已被写硬为 **current runtime serving owner + compatibility anchor + deprecated candidate** 三层姿态；当前仍不得被写成已退场、已删除或已由 local 替代
   - `fact_settlement_ingest_contract_candidate_v1` 已把 planner / serving owner shift 与 fact / settlement owner shift 显式拆开：local evidence 只能进入 evidence / ingest candidate / parity 层，不得直接改 ledger / daily goal / streak / learning day 最终事实
   - `session_entry_and_routing_compat_v1` 已按 compatibility / shadow-only marker 层落地：首页继续 `study_default`，active continuation 继续高优先但不得 silent reroute，auto-routing / mixed routing 仍未进入 runtime
   - `deprecation_markers_and_writeback_plan_v1` 与 `shadow_parity_test_strategy_v1` 已进入最小固定集：四层语义（runtime truth / compatibility-only / deprecated candidate / shadow-only evidence）被要求显式区分，shadow / parity 结果只能作为 evidence，不得写成 owner shift 已完成
   - 本轮未触碰 DB schema / API core semantics / reward-settlement owner / runtime routing；也未把 `review_group` 直接删出运行态
...
### A-OPP-001-026
- **Hypothesis:** `P3.3.6` 可以在不发生 runtime owner shift completed、不改 DB / API core semantics、且不把 shadow / candidate / deprecated-candidate 写成 current runtime truth 的前提下，把 `Compatibility Contract v1 + Shadow-Entry Prep` 落成可执行、可测试、可回写的 very narrow subset。
- **Pass criteria:** `review_group` 的 current owner + compatibility anchor + deprecated candidate 三层姿态被写硬；local-serving candidate 仍停留在 candidate / compatibility / shadow 层；fact / settlement ingest boundary 与 routing compatibility 被写硬；shadow / parity / regression 固定集落地；Room 4 未发生 Major contract drift。
- **Status:** passed on 2026-04-10
...
### E-OPP-001-105
- **Finding:** `R1_P3_3_6_ScopePin_and_Handoff_Pack_v0.1.md` 已将 post-P3.3.5 的下一推进主题正式命名为 `P3.3.6 — Local-Serving Compatibility Contract / Shadow-Mode Entry Round`，并明确本轮是 `Phase 1 — Compatibility Contract Round`，不是 cutover round。
- **Source:** P3.3.6 scope pin / unified handoff entry
- **Date:** 2026-04-10

### E-OPP-001-106
- **Finding:** `R2_P3_3_6_LocalServingCompatibility_and_ShadowModeEntry_Tech_Note_v0.1.md` 明确建议：P3.3.6 最多只应推进到 `Compatibility Contract v1`，只冻结 local-serving candidate 的最小接口层、`review_group` compatibility posture、fact / settlement ingest candidate、routing compatibility、deprecation / write-back 计划与 shadow / parity 测试策略。
- **Source:** Room 2 P3.3.6 technical framing
- **Date:** 2026-04-10

### E-OPP-001-107
- **Finding:** `R3_P3_3_6_LocalServingCompatibility_and_ShadowModeEntry_Rules_Note_v0.1.md` 已将规则层立场写硬：P3.3.6 当前可以进入 `Compatibility Contract v1`，但 `review_group` 仍是 current runtime serving owner；shadow / candidate / deprecated-candidate 只能作为 compatibility / evidence 层，不能被误写成 current runtime truth。
- **Source:** Room 3 P3.3.6 rules note
- **Date:** 2026-04-10

### E-OPP-001-108
- **Finding:** `UI_SPEC_P3_3_6_LocalServingCompatibility_and_ShadowModeEntry_UI_Preflight_v0.1.md` 已将 UI 侧最小合同收口为：Room 5 支持前进一步，但只支持进入 very narrow UI compatibility contract；首页仍 `study_default`，ReviewPage 仍围绕 cloud `review_group`，local-serving shadow 不得变成用户可见事实。
- **Source:** Room 5 P3.3.6 UI preflight
- **Date:** 2026-04-10

### E-OPP-001-109
- **Finding:** `R1_to_R4_P3_3_6_Execution_Handoff_v0.1.md` 已将本轮执行边界写硬为 `Phase 1 / Compatibility Landing + Shadow-Entry Prep`：允许 local-serving candidate contract anchor、`review_group` compatibility posture、fact / settlement ingest boundary、routing compatibility、deprecation markers 与 shadow / parity 测试策略落地，但继续禁止 runtime owner shift、local-serving cutover、DB schema rewrite 与 API core semantics rewrite。
- **Source:** Room 1 → Room 4 P3.3.6 execution handoff
- **Date:** 2026-04-10

### E-OPP-001-110
- **Finding:** user 最新转述：Room 4 已完成 `P3.3.6` 的开发 / 实施；按本轮 handoff 的完成定义，这代表 `Compatibility Contract v1` 的 very narrow landing 已完成，可进入 Room 1 absorb / close 判断。
- **Source:** User relay + P3.3.6 execution handoff Done criteria
- **Date:** 2026-04-10
...
- **Why:** User 已直接拍板下一方向；Room 1 需要把该方向收入口线程，并明确本轮先做 `Compatibility Contract v1 + Shadow-Entry Prep`，而不是直接进入 runtime owner shift / local-serving cutover。
- **Approver:** Room 1 / User

### D-OPP-001-089
- **Decision:** Room 1 接受 `R1_to_R4_P3_3_6_Execution_Handoff_v0.1.md` 所定义的 very narrow execution layer：只进入 `Phase 1 / Compatibility Landing + Shadow-Entry Prep`，允许 local-serving candidate、`review_group` compatibility posture、fact / settlement ingest boundary、routing compatibility、deprecation markers 与 shadow / parity 测试策略落地，但不允许 cutover。
- **Why:** Room 2 / Room 3 / Room 5 的专项输入已被统一收口为一份短而硬的执行单，足以支撑 Room 4 在不触碰 runtime owner shift、DB / API core semantics、reward-settlement owner 与 runtime routing 的前提下完成本轮实施。
- **Approver:** Room 1

### D-OPP-001-090
- **Decision:** Room 1 将运行态 BR / UI 主文件入口更新为 `BR-OPP-001_v0.2.7.md` 与 `UI_SPEC_v0.2.7.md`。
- **Why:** User 已明确告知 BR / UI 主文件完成回写；`v0.2.7` 已以单文件 merged baseline 形态吸收 P3.3.5 closeout 与 P3.3.6 的 compatibility / shadow-entry write-back，足以替代此前 `v0.2.6` 作为当前运行态 BR / UI 入口。
- **Approver:** Room 1

### D-OPP-001-091
- **Decision:** Room 1 在当前主线程中吸收 `P3.3.6` 为 **current-round closed**。
- **Why:** User 已向 Room 1 转述：Room 4 已完成 `P3.3.6` 的开发 / 实施；而本轮 handoff 的完成定义本身只要求 `Compatibility Contract v1` 的 very narrow subset 落地、current runtime truth 保持不变、`review_group` 三层姿态被写硬、fact / settlement ingest boundary 与 routing compatibility 被写硬，以及 shadow / parity / regression 固定集落地，不要求发生 cutover。
- **Approver:** Room 1 / User relay

## 6) Gate Gaps (for MAIN reference)

### G-OPP-001-001
- **Gap:** `post-P3.3.6` 的下一轮产品 / 技术 focus 尚未被 Room 1 / User 正式拍板。
- **Impact:** 当前可接受 P3.3.6 close，但项目若不尽快 pin 新 focus，会再次进入“当前轮已关单、下一轮未起单”的推进真空。
- **Owner:** Room 1 / User
- **Priority:** Critical
...
- **Impact:** P3.3.6 已把 local-serving candidate、`review_group` compatibility posture、fact / settlement ingest boundary、routing compatibility、deprecation markers 与 shadow / parity 固定集往前推一拍，但更深的 owner-shift / cutover 仍必须单开下一轮，不可在当前 closeout 后静默扩写。
...
- **Impact:** 当前 P3.3.6 允许 close，因为本轮仍明确未触碰 DB schema / API core semantics；但若下一轮继续 owner-shift 方向，Room 2 必须单开新的 DB / API compatibility + ingest + migration candidate round，避免治理层与运行态再度脱节。
- **Owner:** Room 2 / Room 1
- **Priority:** High
...
1. **Owner=Room 1 / User | ETA=Next round | Done=拍板 `post-P3.3.6` 的下一推进主题，并明确是继续进入 owner-shift Phase 2（limited execution / shadow mode / DB-API compatibility+ingest）、还是切到别的产品主题 | Action=让项目从 “P3.3.6 已 close” 平滑切到下一推进主题**

2. **Owner=Room 2 | ETA=If owner-shift direction continues | Done=给出下一轮 `DB / API compatibility + ingest + migration candidate` 的范围、红线、shadow-entry 前置条件与 staged rollout 建议 | Action=为可能的 P3.3.7 / Phase 2 owner-shift round 准备技术入口**
...
- 当前最重要的新事实是：**`P3.3.6` 已按 `Compatibility Contract v1 + Shadow-Entry Prep` 范围实施完成，并被 Room 1 吸收到主线程。**
- 本轮真正落地的是：local-serving candidate 仍停留在 candidate / compatibility / shadow 层，`review_group` 的 current owner + compatibility anchor + deprecated candidate 三层姿态被写硬，fact / settlement ingest boundary 与 routing compatibility 被写硬，shadow / parity / regression 固定集被要求落地。
- `current runtime truth` 继续保持不变：ReviewPage 继续围绕 cloud `review_group`，首页继续保持 `study_default`，自动路由与 cutover 仍未进入运行态。
- 当前 active BR / UI baseline 已升级为 `BR-OPP-001_v0.2.7.md` 与 `UI_SPEC_v0.2.7.md`；DB / API baseline 继续保持 `v0.2.1`。
- 下一治理动作不是在 P3.3.6 内静默继续 owner shift，而是 **由 Room 1 / User 拍板 post-P3.3.6 下一轮主题；若继续此方向，则单开下一轮 DB/API compatibility + ingest + shadow-entry candidate round。**
