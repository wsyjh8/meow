/**
 * One-shot batch uploader: temp/out/mp3/example → COS audio/v1/examples
 *
 * Features:
 *   - Checkpoint file (example-progress.json) records uploaded rel paths.
 ...
 *
 * Usage:
 *   npx ts-node scripts/upload-examples-mp3-to-cos.ts --dry-run   # preview
 *   npx ts-node scripts/upload-examples-mp3-to-cos.ts --commit    # upload
 */
...
const SRC_DIR = path.join(PROJECT_ROOT, 'temp', 'out', 'mp3', 'example');
const COS_PREFIX = 'audio/v1/examples';
const CHECKPOINT_FILE = path.join(PROJECT_ROOT, 'temp', 'out', 'mp3', 'example-progress.json');
...
async function main(): Promise<void> {
  ...
  if (!isDryRun && !isCommit) {
    console.error(
      'Usage:\n' +
        '  npx ts-node scripts/upload-examples-mp3-to-cos.ts --dry-run\n' +
        '  npx ts-node scripts/upload-examples-mp3-to-cos.ts --commit',
    );
    process.exit(2);
  }
