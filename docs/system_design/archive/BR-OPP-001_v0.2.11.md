...
**Base merge:** `BR-OPP-001_v0.2.10.md` + `P3.3.9 closeout inputs`  
**Merge policy:** 保留 `v0.2.10` 的 full BR 结构，并将 P3.3.9 已收口的规则吸收为单文件可读的增量主文档候选
...
- `BR-OPP-001_v0.2.10.md`（latest BR review basis / patch base for this round）
...
> Note:
> 1. 本稿以上述 current review basis 为准，并以 `P3.3.9 — First Very Narrow Cutover Round` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.10.md` 的 full BR 结构上，增量吸收 P3.3.9 已收口、且已被 Room 1 同意进入主 BR 回写的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.9 规则；first-cutover / retained-anchor / rollback / hold / seam-candidate 结果仍不得被写成 runtime owner shift completed、ReviewPage local-serving full runtime cutover、`review_group` 真退场、cleanup / baseline uplift、auto-routing runtime、unified planner / planner merge、DB schema rewrite、API core semantics rewrite 等既成事实。
...
- **This file:** `BR-OPP-001_v0.2.11.md`
- **Owner:** Room 3
