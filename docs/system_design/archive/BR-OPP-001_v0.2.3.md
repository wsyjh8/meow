...
**Base merge:** `BR-OPP-001_v0.2.2.md` + `P3.3.1 closeout inputs`  
**Merge policy:** 保留 `v0.2.2` 的 full BR 结构，并将 P3.3.1 已收口的规则吸收为单文件可读的增量主文档候选
...
> Note:
> 1. 本稿以上述 current active runtime basis 为准，并以 `P3.3.1 — 收尾 / 体验补强` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.2.md` 的 full BR 结构上，增量吸收 P3.3.1 已收口的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.1 规则；完整 SRS / planner owner、session 自动分流、统一学习页等继续保持 pending。
...
6. 首页点击“背单词”后的 session 启动合同（新词 / 复习 / 混合 / 自动分流）
7. 云端 `review_group` 与本地 FSRS 的最终 planner owner / 收敛方向
...
- **This file:** `BR-OPP-001_v0.2.3.md`
- **Owner:** Room 3
...
- **Runtime note:** 本稿以上一版 active BR `BR-OPP-001_v0.2.2.md` 为 base，并吸收 P3.3.1 已收口的 wording / bridge / defer 决策；**current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **P3.3.1 note:** 本版继续在 P3.3 第一拍基础上，吸收 P3.3.1 已收口的规则：final wording freeze、forbidden fact-copy、ReviewPage bridge 的 `controlled best-effort` 边界、以及 `previewDurations` 当前轮继续 deferred；session 启动合同、planner owner、统一学习页、完整 SRS / planner 继续保持 pending。
- **Maintenance rule:**
