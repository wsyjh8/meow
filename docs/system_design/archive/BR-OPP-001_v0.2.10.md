...
**Base merge:** `BR-OPP-001_v0.2.10.md` + `P3.3.8 closeout inputs`  
**Merge policy:** 保留 `v0.2.8` 的 full BR 结构，并将 P3.3.7 已收口的规则吸收为单文件可读的增量主文档候选
...
- `BR-OPP-001_v0.2.9.md`（latest BR review basis / patch base for this round）
...
> Note:
> 1. 本稿以上述 current review basis 为准，并以 `P3.3.8 — Phase 3 Gate / Cutover-Decision + DB/API Candidate Round` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.9.md` 的 full BR 结构上，增量吸收 P3.3.8 已收口、且已被 Room 1 接受进入主 BR 回写的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.8 规则；gate / candidate / migration 结果仍不得被写成 runtime owner shift completed、ReviewPage local-serving runtime cutover、`review_group` 退出运行态、auto-routing runtime、unified planner / planner merge、DB schema rewrite、API core semantics rewrite 等既成事实。
...
17. Phase 3 gate 通过后，very narrow limited cutover candidate 的最小执行子集究竟先切哪一段
18. DB/API seam candidate 何时才允许从 candidate contract 升格到 active baseline 或 core semantics rewrite
19. shadow / parity internal-only 结果未来是否允许转成用户可见的 migration messaging / mode declaration
20. 用户可见 owner-shift / mode-switch / cutover 宣告何时才允许出现
...
- **This file:** `BR-OPP-001_v0.2.10.md`
- **Owner:** Room 3
...
- **Runtime note:** 本稿以上一版 candidate BR `BR-OPP-001_v0.2.8.md` 为 base，并吸收 P3.3.7 已收口的 `shadow_execution_scope_v1`、`shadow_result_visibility_v1`、`mismatch_severity_rule_set_v1`、`shadow_acceptance_gate_v1`、`fact_copy_guardrails_v1` 与 `shadow_to_phase3_gate_v1` 的最小合同；**current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **P3.3.7 note:** 本版继续在 P3.3 / P3.3.1 / P3.3.2 / P3.3.3 / P3.3.4 / P3.3.5 / P3.3.6 基础上，吸收 P3.3.7 已收口的规则：local-serving / fact-ingest / routing 的 shadow run 当前只进入 limited execution 的 evidence 层；`review_group` 继续保持 current runtime owner，同时作为 shadow compare baseline；mismatch 分级、acceptance gate、fact-copy guardrails 与 `shadow_to_phase3_gate_v1` 被正式写硬；runtime owner shift completed、ReviewPage local-serving cutover、`review_group` 退出运行态、auto-routing runtime、DB/API core rewrite、planner merge / unified planner 与任何用户可见 owner-shift 宣告继续保持 pending。
- **Maintenance rule:**
