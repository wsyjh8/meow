...
**Base merge:** `BR-OPP-001_v0.2.5.md` + `P3.3.4 closeout inputs`  
...
- `BR-OPP-001_v0.2.5.md`（current runtime active BR baseline）
...
> Note:
> 1. 本稿以上述 current active runtime basis 为准，并以 `P3.3.4 — Preview Re-entry + Stronger Bridge Round` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.5.md` 的 full BR 结构上，增量吸收 P3.3.4 已收口的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.4 规则；Study + Review preview、preview explanation system、must-succeed bridge、planner owner shift、planner merge / unified planner、auto-routing、DB / API core change 等继续保持 pending。
...
- **This file:** `BR-OPP-001_v0.2.6.md`
- **Owner:** Room 3
...
- **Runtime note:** 本稿以上一版 candidate BR `BR-OPP-001_v0.2.5.md` 为 base，并吸收 P3.3.4 已收口的 `preview_durations_reentry_contract_v1` 与 `reviewpage_stronger_bridge_contract_v1` 的最小合同；**current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **P3.3.4 note:** 本版继续在 P3.3 / P3.3.1 / P3.3.2 / P3.3.3 基础上，吸收 P3.3.4 已收口的规则：`preview_durations_reentry_contract_v1`（StudyPage only / estimated hint / local FSRS preview candidate / 非计划事实）与 `reviewpage_stronger_bridge_contract_v1`（stronger ensure / init + observability + minimal repair path + 仍然 non-blocking）；Study + Review preview、完整 explanation system、must-succeed bridge、planner merge / unified planner、auto-routing 与完整复习规划继续保持 pending。
- **Maintenance rule:**
