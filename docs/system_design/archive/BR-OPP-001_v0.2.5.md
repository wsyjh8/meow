...
**Base merge:** `BR-OPP-001_v0.2.4.md` + `P3.3.3 closeout inputs`  
**Merge policy:** 保留 `v0.2.4` 的 full BR 结构，并将 P3.3.3 已收口的规则吸收为单文件可读的增量主文档候选
...
- `BR-OPP-001_v0.2.3.md`（current runtime active BR baseline）
...
> Note:
> 1. 本稿以上述 current active runtime basis 为准，并以 `P3.3.3 — Review Planning Contract v1 / SRS Boundary Round` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.4.md` 的 full BR 结构上，增量吸收 P3.3.3 已收口的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.3 规则；完整 SRS / full priority scoring / exact group size contract / planner merge / auto-routing / `previewDurations` future re-entry 等继续保持 pending。
...
### PD-013 `previewDurations` / interval preview 的未来重新进入条件
- **Question:** 在后续轮次中，`previewDurations` / interval preview 何时才具备进入 active contract 的条件？
- **Current state:** Pending（当前轮已正式 deferred）
- **Room 3 default:** 若未来进入，应单独冻结“解释性 preview”边界、来源单一性与 false-fact 禁区，避免把 preview 写成后端已确认事实。
...
- **This file:** `BR-OPP-001_v0.2.5.md`
- **Owner:** Room 3
...
- **Runtime note:** 本稿以上一版 candidate BR `BR-OPP-001_v0.2.4.md` 为 base，并吸收 P3.3.3 已收口的 `review_readiness_policy_v1` / `review_priority_policy_v1` / `review_group_generation_policy_v1` / `schedule_source_contract_v1` / `previewDurations` continued defer 决策；**current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **P3.3.3 note:** 本版继续在 P3.3 / P3.3.1 / P3.3.2 基础上，吸收 P3.3.3 已收口的规则：`review_readiness_policy_v1`、`review_priority_policy_v1`、`review_group_generation_policy_v1`、`schedule_source_contract_v1`、以及 `previewDurations` 当前轮继续 deferred；full priority scoring / exact group size / planner merge / auto-routing / preview explanation / 完整 SRS 继续保持 pending。
- **Maintenance rule:**
