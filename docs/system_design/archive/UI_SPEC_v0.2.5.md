...
- **Purpose:** 在 `UI_SPEC_v0.2.1.md` 已成为当前 runtime active UI baseline 的前提下，按 **P3.3 First Pass Closed** 的已收口结果，对首页“背单词”入口、Study / Review 4 按钮交互、页面承接关系与文案事实边界做 **增量回写**。  
  本稿不是重写整份 UI SPEC，也不是把 P3.3 / P3.3.1 / P3.3.2 / P3.3.3 preflight / candidate 全量升格；它只吸收 **已经被 Room 1 / Room 2 / Room 3 / Room 4 收口到足以影响后续开发、维护与新功能判断** 的 P3.3、P3.3.1、P3.3.2 与 P3.3.3 页面现实。
...
- **继续保留为 pending / candidate，不直接升格：**
  ...
  - `previewDurations` 的 future active contract / 解释增强
...
### 0.1 运行时说明
- 当前推进层 SSOT 中，**active UI baseline 仍是 `UI_SPEC_v0.2.3.md`**。
- `UI_SPEC_v0.2.4.md` 是 P3.3 + P3.3.1 + P3.3.2 write-back candidate。
- 本稿是 **在 `v0.2.4` 基础上继续吸收 P3.3.3 closeout 的 next-step UI baseline candidate / runtime-baseline update candidate**。
- 在 Room 1 将本稿正式吸收到 `Main / STATUS` 前，`v0.2.3` 仍是当前运行态依据。
- 但从 Room 5 视角，后续开发、维护与新功能设计，**应开始优先参考本稿中的 P3.3 + P3.3.1 + P3.3.2 + P3.3.3 增量回写部分**，再向上服从当前 active PRD / BR / DB / API / ORG / Role Cards。
## 1. 输入依据
...
### 1.4 吸收边界
1. 只吸收 **P3.3 / P3.3.1 / P3.3.2 / P3.3.3 已形成页面现实或已被 Room 1 正式冻结为最小合同** 的内容
...
6. 已经能稳定复现、且会影响新功能判断的页面状态与防误报边界
...
### 2.3 一句话原则
> **把 P3.3 First Pass、P3.3.1 closeout、P3.3.2 最小合同层与 P3.3.3 的 review-planning very narrow UI contract 吸收进主 UI SPEC；把完整 review planning、`previewDurations` 的 future 合同、mixed / auto-routing runtime contract、以及更强 bridge contract 继续留在 Pending。**
## 3. 总体信息架构（吸收后口径）
...
- 当前页面已正式吸收的现实：
  ...
  - 本地 FSRS bridge 当前只收口到 `controlled best-effort`
...
- 当前页面文案边界：
  ...
  - 不得写“复习规划已更新 / 已自动调整 / 本地计划已同步 / 已切到最佳学习路径”
  - 只有实际 `groupCompleted` / settlement 条件满足时，才允许展示相应结果层反馈
- 因此，后续开发维护时：
  - 可把 `ReviewPage` 视为 **4 按钮已接入，review_group 仍是主真相源，local FSRS 只是 side-effect owner** 的页面
  - 但**不能把当前 `controlled best-effort` bridge 误写成更强 planner contract，也不能把 planner owner split 写成 planner merge 已成立**
## 5.8 SessionPage
**定位：** 专注 Session  
**正式吸收结论：**
