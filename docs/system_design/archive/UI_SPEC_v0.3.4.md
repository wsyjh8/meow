...
- **Purpose:** 在 `UI_SPEC_v0.2.1.md` 已成为当前 runtime active UI baseline 的前提下，按 **P3.3 First Pass Closed** 的已收口结果，对首页“背单词”入口、Study / Review 4 按钮交互、页面承接关系与文案事实边界做 **增量回写**。  
  本稿不是重写整份 UI SPEC，也不是把 P3.3 / P3.3.1 / P3.3.2 / P3.3.3 / P3.3.4 / P3.3.5 / P3.3.6 / P3.3.7 / P3.3.8 / P3.3.9 / P3.3.10 / P3.3.11 / P3.3.12 preflight / candidate 全量升格；它只吸收 **已经被 Room 1 / Room 2 / Room 3 / Room 4 收口到足以影响后续开发、维护与新功能判断** 的 P3.3、P3.3.1、P3.3.2、P3.3.3、P3.3.4、P3.3.5、P3.3.6、P3.3.7、P3.3.8、P3.3.9、P3.3.10、P3.3.11 与 P3.3.12 页面现实。
...
### 0.1 运行时说明
- 当前推进层 / 本轮 review basis 中，`UI_SPEC_v0.3.3.md` 已作为当前 UI write-back / review basis 使用。
- 本稿是 **在 `v0.3.3` 基础上继续吸收 P3.3.12 closeout 的 next-step UI baseline candidate / runtime-baseline update candidate**。
- 在 Room 1 将本稿正式吸收到 `Main / STATUS` 前，`v0.3.3` 仍是当前 write-back / runtime-baseline update basis。
- 但从 Room 5 视角，后续开发、维护与新功能设计，**应开始优先参考本稿中的 P3.3 + P3.3.1 + P3.3.2 + P3.3.3 + P3.3.4 + P3.3.5 + P3.3.6 + P3.3.7 + P3.3.8 + P3.3.9 + P3.3.10 + P3.3.11 + P3.3.12 增量回写部分**，再向上服从当前 active PRD / BR / DB / API / ORG / Role Cards。
## 1. 输入依据
...
### 1.4 吸收边界
1. 只吸收 **P3.3 / P3.3.1 / P3.3.2 / P3.3.3 / P3.3.4 / P3.3.5 / P3.3.6 / P3.3.7 / P3.3.8 / P3.3.9 / P3.3.10 / P3.3.11 / P3.3.12 已形成页面现实或已被 Room 1 正式冻结为最小合同** 的内容
...
### 2.3 一句话原则
> **把 P3.3 First Pass、P3.3.1 closeout、P3.3.2 最小合同层、P3.3.3 的 review-planning very narrow UI contract、P3.3.4 的 preview re-entry / stronger bridge 最小 UI 合同、P3.3.5 的 target-state / compatibility / semantic rewrite 边界、P3.3.6 的 shadow-compatible local-serving / review-group compatibility / fact-boundary contract、P3.3.7 的 internal-only shadow execution / runtime-truth guardrails / shadow visibility gate、P3.3.8 的 Phase 3 gate / limited cutover subset candidate / source-neutral UI migration path、P3.3.9 的 first very narrow cutover preflight / retained-anchor / rollback-hold-observability、P3.3.10 的 fuller cutover judgment / exit-candidate judgment / uplift-readiness judgment、P3.3.11 的 execution-ready subset / exit-candidate / uplift-readiness，以及 P3.3.12 的 absorb-candidate judgment / true-exit-gate judgment / uplift-absorb judgment 吸收进主 UI SPEC；把 full cutover、true exit、uplift absorbed、homepage route switch、active continuation source switch、final fact owner shift 与 cleanup 继续留在 Pending。**
## 3. 总体信息架构（吸收后口径）
