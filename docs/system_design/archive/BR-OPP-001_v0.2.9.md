...
**Base merge:** `BR-OPP-001_v0.2.8.md` + `P3.3.7 closeout inputs`  
...
- `BR-OPP-001_v0.2.8.md`（latest BR review basis / patch base for this round）
...
> Note:
> 1. 本稿以上述 current review basis 为准，并以 `P3.3.7 — Local-Serving Limited Execution / Shadow Mode Round` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.8.md` 的 full BR 结构上，增量吸收 P3.3.7 已收口、且已被 Room 1 同意进入主 BR 回写的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.7 规则；shadow 结果仍只到 evidence 层，runtime owner shift completed、ReviewPage local-serving runtime cutover、`review_group` 退出运行态、auto-routing runtime、unified planner / planner merge、DB / API core rewrite 等继续保持 pending。
...
17. P3.3.7 的 shadow evidence 何时足以支持进入 Phase 3 / cutover 判断
18. shadow / parity internal-only 结果未来是否允许转成用户可见的 migration messaging / mode declaration
...
- **This file:** `BR-OPP-001_v0.2.9.md`
- **Owner:** Room 3
