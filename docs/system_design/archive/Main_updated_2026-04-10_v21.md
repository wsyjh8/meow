...
- **Links / Entrances**
  ...
  - Room 4: `ROOM04_治理版_v0.2`
  - Room 5: `ROOM05_ROLE_CARD_UI_UX_v0.2.1`
  - BR: `BR-OPP-001_v0.2.3.md`
  - DB: `背单词喵喵app_DB设计草案_v0.2.1.md`
  - API: `背单词喵喵app_API设计草案_v0.2.1.md`
  - UI: `UI_SPEC_v0.2.3.md`

## 1) One-liner

面向**容易半途放弃背单词、但对可爱陪伴与轻养成有偏好的学习用户**，在**日常背词、复习、签到和碎片时间学习**场景下，解决**难开始、难坚持、反馈弱、长期留存低**的问题，通过**学习主机制 + 喵喵副机制承接**，带来**更高的打开率、完成率、复习率与长期陪伴式学习体验**；当前 `P3.1 — Local Progress + Cloud Backup` 的代码与 BR / DB / API / UI 文档已完成本轮对齐，Room 1 已将 runtime baseline 从旧 `v0.1.x` 正式提升到 `v0.2.1`；在此基础上，`P3.3` 已整体关单，而 `P3.3.2` 又把复习规划继续深化收口到 **minimal contract**：`session_entry_policy_v1` 已冻结为 `home_word_entry = study_default`，active `review_group` continuation 继续高优先但只能独立承接、不得 silent reroute；`planner_owner_split_v1` 已冻结为 **ReviewPage 由 cloud `review_group` 承担 serving truth owner，本地 FSRS 承担 device-side scheduling owner，复习提交流程继续 `cloud-first + local side-effect`**；当前 Room 1 可将 `P3.3.2` 判定为 **current-round closed**。
...
### G-OPP-001-001
- **Gap:** `post-P3.3.2` 的下一轮产品 / 技术 focus 尚未被 Room 1 / User 正式拍板。
- **Impact:** 当前可接受 P3.3.2 close，但项目若不尽快 pin 新 focus，会再次进入“当前轮已关单、下一轮未起单”的推进真空。
- **Owner:** Room 1 / User
- **Priority:** Critical

### G-OPP-001-002
- **Gap:** `P3.3.2` 的 minimal-contract 事实目前已被吸收到 Main / STATUS 与专项 handoff，但尚未写回下一版 BR / UI 主文档。
- **Impact:** 不阻塞当前推进，但若长期不合并，会形成“主线程已前进、规则与 UI 主文档仍停在上一轮”的阅读分裂。
- **Owner:** Room 1 / Room 3 / Room 5
- **Priority:** High

### G-OPP-001-003
- **Gap:** ReviewPage 的 residual bridge issue 仍保留为后续技术议题；当前虽不阻塞 P3.3.2 close，但未来若继续深化本地 FSRS 与云端 review_group 收敛，仍需明确开新 round 处理。
- **Impact:** 会影响后续更深 review planning / planner convergence 轮次的进入成本。
- **Owner:** Room 2 / Room 4
- **Priority:** Minor
...
1. **Owner=Room 1 / User | ETA=Next round | Done=拍板 `post-P3.3.2` 的下一轮 focus，并明确是继续做 review planning deepening、做技术 round，还是转向别的产品主题 | Action=让项目从 “P3.3.2 已 close” 平滑切到下一推进主题**

2. **Owner=Room 1 / Room 3 / Room 5 | ETA=Later | Done=把 `P3.3.2` 的 minimal-contract（`session_entry_policy_v1` + `planner_owner_split_v1`）写回下一版 BR / UI 主文档 | Action=让治理层主文档与推进层运行态重新同轨**

3. **Owner=Room 1 | ETA=Later | Done=更新 archive / retained-reference 处理策略，清理 `v0.1.x` 旧入口噪音 | Action=让 runtime active entry 更干净**

## 8) Notes (≤5 lines)

- 当前最重要的新事实是：**`P3.3.2` 已按最小合同层实施完成，并被 Room 1 吸收到主线程。**
- 本轮真正落地的是：`home_word_entry = study_default` + `planner_owner_split_v1`。
- 当前 runtime 真实口径已明确：ReviewPage 继续 `cloud-first + local side-effect`，但并未进入 auto-routing / unified planner。
- 本轮仍未触碰 DB schema / API 核心语义 / `review_group` 最小合同 / planner owner 基线。
- 下一治理动作不是继续补 P3.3.2，而是 **拍板 post-P3.3.2 下一轮 focus，并决定是否做 BR / UI 主文档回写。**

## 9) Working Rule

> **Main 现在必须反映真实主线程：P1 / P2 / Option A / Option A.1 / Option B / Option C / P3 / P3.1 / P3.3 / P3.3.2 均已完成当前轮关单；其中 P3.3.2 只把入口语义与 owner split 收成最小合同，并未把完整复习规划做厚。下一步不是继续在 P3.3.2 内补脑，而是由 Room 1 / User 明确 pin `post-P3.3.2` 的下一推进主题。**
