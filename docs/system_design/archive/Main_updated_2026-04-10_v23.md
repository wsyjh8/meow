...
- **Incremental update note:** 本版以 `Main_updated_2026-04-10_v21.md` 为 base，仅增量吸收 `P3.3.3 closeout` 与 `BR / UI v0.2.5` runtime-baseline update；未被本轮触发的既有主线程内容保持不动。
...
面向**容易半途放弃背单词、但对可爱陪伴与轻养成有偏好的学习用户**，在**日常背词、复习、签到和碎片时间学习**场景下，解决**难开始、难坚持、反馈弱、长期留存低**的问题，通过**学习主机制 + 喵喵副机制承接**，带来**更高的打开率、完成率、复习率与长期陪伴式学习体验**；当前 `P3.1 — Local Progress + Cloud Backup` 的代码与 BR / DB / API / UI 文档已完成本轮对齐，Room 1 已将 runtime baseline 从旧 `v0.1.x` 正式提升到 `v0.2.1`；在此基础上，`P3.3` 已整体关单，而 `P3.3.2` 又把复习规划继续深化收口到 **minimal contract**：`session_entry_policy_v1` 已冻结为 `home_word_entry = study_default`，active `review_group` continuation 继续高优先但只能独立承接、不得 silent reroute；`planner_owner_split_v1` 已冻结为 **ReviewPage 由 cloud `review_group` 承担 serving truth owner，本地 FSRS 承担 device-side scheduling owner，复习提交流程继续 `cloud-first + local side-effect`**；当前 Room 1 可将 `P3.3.2` 判定为 **current-round closed**。在此基础上，`P3.3.3` 又把复习规划继续向 very narrow minimal contract 推进一层：页面级 `review_readiness_policy_v1` 继续以 cloud aggregate / review-serving layer 为 truth，`review_priority_policy_v1` 只冻结 hierarchy，`review_group_generation_policy_v1` 只冻结 owner + gating + non-overclaim，`schedule_source_contract_v1` 只冻结 truth split，`previewDurations` 继续 deferred；当前 Room 1 可将 `P3.3.3` 判定为 **current-round closed**。
...
### G-OPP-001-001
- **Gap:** `post-P3.3.3` 的下一轮产品 / 技术 focus 尚未被 Room 1 / User 正式拍板。
- **Impact:** 当前可接受 P3.3.3 close，但项目若不尽快 pin 新 focus，会再次进入“当前轮已关单、下一轮未起单”的推进真空。
- **Owner:** Room 1 / User
- **Priority:** Critical

### G-OPP-001-002
- **Gap:** `P3.3.3` 虽已写回 `BR-OPP-001_v0.2.5.md` 与 `UI_SPEC_v0.2.5.md`，但 `STATUS` 仍需与本次 Main 的运行态更新保持同轨。
- **Impact:** 若推进层只更新 Main、不同步 STATUS，会形成运行态双文件的短期错位。
- **Owner:** Room 1
- **Priority:** High

### G-OPP-001-003
- **Gap:** ReviewPage 的 residual bridge issue 仍保留为后续技术议题。
- **Impact:** 当前不阻塞 P3.3.3 close，但未来若继续深化 planner convergence / stronger bridge contract，仍需单开一轮技术处理。
- **Owner:** Room 2 / Room 4
- **Priority:** Minor
...
1. **Owner=Room 1 / User | ETA=Next round | Done=拍板 `post-P3.3.3` 的下一轮 focus，并明确是继续深化 review planning、转技术 round，还是切到别的产品主题 | Action=让项目从 “P3.3.3 已 close” 平滑切到下一推进主题**

2. **Owner=Room 1 | ETA=Immediately after this Main update | Done=`STATUS` 同步完成到与本版 Main 一致的 runtime active 口径（`P3.3.3 Closed / Next-Focus Pending` + BR/UI `v0.2.5`） | Action=让推进层双文件重新同轨**

3. **Owner=Room 1 | ETA=Later | Done=更新 archive / retained-reference 处理策略，清理 `v0.1.x` 与 `v0.2.2 / v0.2.3` 旧入口噪音 | Action=让 runtime active entry 更干净**

## 8) Notes (≤5 lines)

- 当前最重要的新事实是：**`P3.3.3` 已按 very narrow minimal contract 实施完成，并被 Room 1 吸收到主线程。**
- 本轮真正落地的是：`review_readiness_policy_v1` / `review_priority_policy_v1` / `review_group_generation_policy_v1` / `schedule_source_contract_v1` 的最小合同层。
- `previewDurations` 继续 deferred；当前 runtime 仍未进入 full priority scoring / exact group size / unified planner / auto-routing。
- BR / UI 主文件入口现应以 `BR-OPP-001_v0.2.5.md` 与 `UI_SPEC_v0.2.5.md` 为准。
- 下一治理动作不是继续在 P3.3.3 内补脑，而是 **拍板 post-P3.3.3 下一轮 focus，并同步更新 STATUS。**

## 9) Working Rule

> **Main 现在必须反映真实主线程：P1 / P2 / Option A / Option A.1 / Option B / Option C / P3 / P3.1 / P3.3 / P3.3.2 / P3.3.3 均已完成当前轮关单；其中 P3.3.3 只把 readiness / priority / generation / source split 收成 very narrow minimal contract，并未把完整 review planning / unified planner 做厚。下一步不是继续在 P3.3.3 内补脑，而是由 Room 1 / User 明确 pin `post-P3.3.3` 的下一推进主题。**
