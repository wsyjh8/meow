...
- **Incremental update note:** 本版以 `Main_updated_2026-04-10_v33.md` 为 base，仅增量吸收 `P3.3.13 closeout` 与 `BR / UI v0.2.15 / v0.3.5` runtime-baseline update；未被本轮触发的既有主线程内容保持不动.
...
- **Links / Entrances**
  ...
  - Room 4: `ROOM04_治理版_v0.2`
  - Room 5: `ROOM05_ROLE_CARD_UI_UX_v0.2.1`
  - BR: `BR-OPP-001_v0.2.14.md`
  - DB: `背单词喵喵app_DB设计草案_v0.2.1.md`
  - API: `背单词喵喵app_API设计草案_v0.2.1.md`
  - UI: `UI_SPEC_v0.3.4.md`

## 1) One-liner

面向**容易半途放弃背单词、但对可爱陪伴与轻养成有偏好的学习用户**，在**日常背词、复习、签到和碎片时间学习**场景下，解决**难开始、难坚持、反馈弱、长期留存低**的问题，通过**学习主机制 + 喵喵副机制承接**，带来**更高的打开率、完成率、复习率与长期陪伴式学习体验**；当前 `P3.1 — Local Progress + Cloud Backup` 的代码与 BR / DB / API / UI 文档已完成本轮对齐，Room 1 已将 runtime baseline 从旧 `v0.1.x` 正式提升到 `v0.2.1`；在此基础上，`P3.3` 已整体关单，`P3.3.2`、`P3.3.3`、`P3.3.4` 已分别把复习规划、preview 与 stronger bridge 推进到当前 very narrow minimal-contract 层。进一步地，`P3.3.5` 已完成 **Phase 0 / Compatibility-Prep**，`P3.3.6` 已完成 **Compatibility Contract v1 / Shadow-Entry Prep**，`P3.3.7` 已完成 **Phase 2 / Limited Execution / Shadow Mode**，`P3.3.8` 已完成 **Phase 3 / Gate-Driven Candidate Execution + Migration Prep**，而 `P3.3.9` 则把 owner-shift 方向推进到 **First Very Narrow Cutover**：第一拍只在 ReviewPage 的 **non-continuation serving subset** 上切入极窄 source seam，同时继续保留 `review_group` 作为 current runtime owner + retained fallback anchor，首页继续保持 `study_default`，active continuation 继续独立承接且不得 silent reroute，final fact / settlement truth 继续以后端为准；在此基础上，`P3.3.10` 已进一步把这条线推进到 **Fuller Cutover / review_group Exit-Gate / DB-API Uplift Judgment** 的 very narrow subset：允许把扩大方向判断到 ReviewPage continuity-adjacent serving-adapter family，并把 `review_group` retained-anchor → exit-candidate 的条件、DB/API uplift-judgment-ready seam families、以及 judgment / candidate / runtime-truth 的分层回写顺序写硬，但本轮仍未把 full cutover completed、`review_group` 真退场、active DB/API baseline uplift absorbed、final fact owner shift 或 cleanup 写成当前事实。进一步地，`P3.3.11` 已把这条线继续推进到 **Fuller-Cutover Execution / review_group Exit-Candidate / DB-API Uplift-Readiness** 的 very narrow subset：允许把 widened subset 从纯 judgment 推到 **ReviewPage + 首页 review 承接层** 的 execution-ready subset，并把 `review_group` 继续保留在 current owner + retained fallback anchor + compatibility anchor + deprecated candidate 的并存姿态，只把其推进到 exit-candidate；同时 DB/API 只推进到 uplift-readiness seam families，stronger-ingest 只推进到 execution-ready binding prep，仍未把 full cutover completed、`review_group` true exit、active DB/API baseline uplift absorbed、homepage route switch、active continuation source switch、final fact owner shift 或 cleanup 写成当前事实。进一步地，`P3.3.12` 已把这条线继续推进到 **Fuller-Cutover / True-Exit-Gate / DB-API Uplift-Absorb Judgment** 的 very narrow subset：允许把 widened execution-ready subset 提升到 absorb-candidate judgment，把 `review_group` 推进到 true-exit-gate judgment，把 DB/API 推进到 uplift-absorb judgment-ready seam families，但仍未把 full cutover completed、`review_group` true exit 已生效、active DB/API baseline uplift absorbed、homepage route switch、active continuation source switch、final fact owner shift 或 cleanup 写成当前事实。进一步地，`P3.3.13` 已把这条线继续推进到 **Fuller-Cutover Execution / True-Exit-Candidate / DB-API Uplift-Absorb-Readiness** 的 very narrow subset：允许把 widened subset 从 judgment 再前推进到 **ReviewPage + 首页 review 承接层** 的 execution subset，并把 `review_group` 继续保留在 true-exit-candidate 而非 true exit，把 DB/API 继续保留在 uplift-absorb-readiness 而非 uplift absorbed；但仍未把 full cutover completed、`review_group` true exit、active DB/API baseline uplift absorbed、homepage route switch、active continuation source switch、final fact owner shift 或 cleanup 写成当前事实。当前 Room 1 可将 `P3.3.13` 判定为 **current-round closed**，并将 active BR / UI runtime baselines 正式升级到 `BR-OPP-001_v0.2.15.md` 与 `UI_SPEC_v0.3.5.md`。
...
20. **P3.3.11 — Fuller-Cutover Execution / `review_group` Exit-Candidate / DB-API Uplift-Readiness Round（Closed）**
   - 本轮完成的是 **very narrow execution-ready candidate execution**，不是 full cutover；允许前进一步的 widened subset 只扩大到 **ReviewPage + 首页 review 承接层**，尤其是 ReviewPage continuity-adjacent serving-adapter family 与首页 review helper / summary / no-review-state 的 retained-anchor-aware prep
   - `review_group` 当前继续保持 **current runtime serving owner + retained fallback anchor + compatibility anchor + deprecated candidate**；本轮只把它推进到 **exit-candidate** 层，不进入 true exit，不降成 fallback-only，也不允许 old path cleanup
   - DB / API 当前只进入 **uplift-readiness seam families / marker / migration note / rollback floor / hold note**；active DB/API baseline 继续保持 `v0.2.1`，不进入 schema rewrite / endpoint core semantics rewrite / active uplift absorbed
   - stronger-ingest candidate 只推进到 **execution-ready stronger-candidate binding**；final fact / settlement truth 继续以后端为准，首页继续保持 `study_default`，active continuation 继续独立承接且不得 silent reroute
   - 本轮未发生 runtime owner shift completed、ReviewPage local-serving full runtime cutover、`review_group` true exit、active DB/API baseline uplift absorbed、homepage route switch、active continuation source switch、final fact owner shift、auto-routing runtime、planner merge / unified planner 或 cleanup / old-path purge


21. **P3.3.12 — Fuller-Cutover / True-Exit-Gate / DB-API Uplift-Absorb Judgment Round（Closed）**
   - 本轮完成的是 **fuller-cutover / true-exit-gate / DB-API uplift-absorb judgment** 的 very narrow subset，不是 full cutover；它继续把 widened subset 的推进限制在 **ReviewPage + 首页 review 承接层** 的 judgment artifacts
   - `review_group` 当前继续保持 **current runtime serving owner + retained fallback anchor + compatibility anchor + deprecated candidate**；本轮只进入 **true-exit-gate judgment**，不进入 true exit，不进入 fallback-only，更不允许 old-path cleanup
   - DB / API 当前只进入 **uplift-absorb judgment-ready seam families / seam map / marker / migration note / rollback floor / hold note**；active DB/API baseline 继续保持 `v0.2.1`，不进入 schema rewrite / endpoint core semantics rewrite / uplift absorbed
   - stronger-ingest 当前最多只进入 **absorb-judgment-level candidate**；final fact / settlement truth 继续以后端为准，首页继续保持 `study_default`，active continuation 继续独立承接且不得 silent reroute
   - 本轮未发生 runtime owner shift completed、ReviewPage local-serving full runtime cutover、`review_group` true exit、active DB/API baseline uplift absorbed、homepage route switch、active continuation source switch、final fact owner shift、auto-routing runtime、planner merge / unified planner 或 cleanup / old-path purge

22. **P3.3.13 — Fuller-Cutover Execution / True-Exit-Candidate / DB-API Uplift-Absorb-Readiness Round（Closed）**
   - 本轮完成的是 **fuller-cutover execution / true-exit-candidate / DB-API uplift-absorb-readiness** 的 very narrow subset，不是 full cutover；允许前进一步的 widened subset 仍只扩大到 **ReviewPage + 首页 review 承接层** 的 execution subset
   - `review_group` 当前继续保持 **current runtime serving owner + retained fallback anchor + compatibility anchor + deprecated candidate**；本轮只把它推进到 **true-exit-candidate**，不进入 true exit，不进入 fallback-only，也不允许 old-path cleanup
   - DB / API 当前只进入 **uplift-absorb-readiness seam families / marker / migration note / rollback floor / hold note**；active DB/API baseline 继续保持 `v0.2.1`，不进入 schema rewrite / endpoint core semantics rewrite / uplift absorbed
   - stronger-ingest 当前最多只进入 **absorb-readiness-level binding prep**；final fact / settlement truth 继续以后端为准，首页继续保持 `study_default`，active continuation 继续独立承接且不得 silent reroute
   - 本轮未发生 runtime owner shift completed、ReviewPage local-serving full runtime cutover、`review_group` true exit、active DB/API baseline uplift absorbed、homepage route switch、active continuation source switch、final fact owner shift、auto-routing runtime、planner merge / unified planner 或 cleanup / old-path purge
...
### E-OPP-001-125
- **Finding:** `R1_P3_3_11_ScopePin_and_Handoff_Pack_v0.2.md` 已把 post-P3.3.10 的下一方向正式命名为 `P3.3.11 — Fuller-Cutover Execution / review_group Exit-Candidate / DB-API Uplift-Readiness Round`，并明确本轮只允许把 widened subset 推到 execution-ready 一小层，不把 true exit / uplift absorbed / cleanup 写成已生效事实。
- **Source:** Room 1 P3.3.11 scope pin handoff
- **Date:** 2026-04-11

### E-OPP-001-126
- **Finding:** `R2_P3_3_11_FullerCutoverExecution_ExitCandidate_and_DBUpliftReadiness_Tech_Note_v0.1.md`、`R3_P3_3_11_FullerCutoverExecution_ExitCandidate_and_DBUpliftReadiness_Rules_Note_v0.1.md` 与 `UI_SPEC_P3_3_11_FullerCutoverExecution_ExitCandidate_and_DBUpliftReadiness_UI_Preflight_v0.1.md` 已共同收口：P3.3.11 可以前进一步，但只应进入 `fuller-cutover execution / exit-candidate / uplift-readiness` 的 very narrow subset，不得写成 full cutover completed、`review_group` true exit、active DB/API baseline uplift absorbed 或 cleanup。
- **Source:** Room 2 / Room 3 / Room 5 P3.3.11 execution-preflight inputs
- **Date:** 2026-04-11

### E-OPP-001-127
- **Finding:** `R1_to_R4_P3_3_11_Execution_Handoff_v0.1.md` 已把本轮执行边界写硬为：只在 ReviewPage + 首页 review 承接层扩大 execution-ready subset，继续把 `review_group` 保留为 current owner + retained fallback anchor，继续把 DB/API uplift 停留在 uplift-readiness seam families，并继续守住 final fact / settlement cloud-owner 边界。
- **Source:** Room 1 → Room 4 P3.3.11 execution handoff
- **Date:** 2026-04-11

### E-OPP-001-128
- **Finding:** `BR-OPP-001_v0.2.13.md` 与 `UI_SPEC_v0.3.3.md` 已将 P3.3.11 已收口的规则与页面现实增量回写进单文件 merged baseline，且都已明确标注 *ready for Room 1 runtime-baseline update*。
- **Source:** BR / UI v0.2.13 / v0.3.3 write-back candidates
- **Date:** 2026-04-11
...
### D-OPP-001-105
- **Decision:** Room 1 接受 `R1_P3_3_11_ScopePin_and_Handoff_Pack_v0.2.md`，并正式将 post-P3.3.10 的下一推进主题命名为 `P3.3.11 — Fuller-Cutover Execution / review_group Exit-Candidate / DB-API Uplift-Readiness Round`。
- **Why:** P3.3.10 已把 fuller cutover / exit-gate / uplift judgment 收硬；若继续 owner-shift 方向，下一步不再只是 judgment，而是必须把 widened subset 推到 execution-ready 一小层，同时继续把 true exit / uplift absorbed / cleanup 后置。
- **Approver:** Room 1

### D-OPP-001-106
- **Decision:** Room 1 接受 `R1_to_R4_P3_3_11_Execution_Handoff_v0.1.md` 所定义的本轮 very narrow execution layer：只进入 ReviewPage + 首页 review 承接层的 execution-ready subset、`review_group` exit-candidate、DB/API uplift-readiness 与 stronger-ingest execution-ready binding prep，不得把 full cutover、`review_group` true exit、active DB/API uplift absorbed 或 cleanup 写成当前事实。
- **Why:** Room 2 / Room 3 / Room 5 的专项输入已被统一收口为一份短而硬的执行单，足以支撑 Room 4 在不触碰 homepage route、active continuation、final fact owner、true exit、active uplift 与 cleanup bundling 的前提下完成本轮实施。
- **Approver:** Room 1

### D-OPP-001-107
- **Decision:** Room 1 将运行态 BR / UI 主文件入口更新为 `BR-OPP-001_v0.2.13.md` 与 `UI_SPEC_v0.3.3.md`，并在当前主线程中吸收 `P3.3.11` 为 **current-round closed**。
- **Why:** User 已明确告知 BR / UI 主文件完成回写，且已转述 Room 4 完成 `P3.3.11` 的开发 / 实施；而本轮 handoff 的完成定义本身只要求 execution-ready / exit-candidate / uplift-readiness 的 very narrow subset 落地，不要求发生 full cutover completed、`review_group` true exit、active DB/API baseline uplift absorbed 或 cleanup。
- **Approver:** Room 1 / User relay


### D-OPP-001-108
- **Decision:** Room 1 接受 `R1_P3_3_13_ScopePin_and_Handoff_Pack_v0.1.md`，并正式将 post-P3.3.12 的下一推进主题命名为 `P3.3.13 — Fuller-Cutover Execution / True-Exit-Candidate / DB-API Uplift-Absorb-Readiness Round`。
- **Why:** P3.3.12 已把 fuller-cutover / true-exit-gate / uplift-absorb judgment 收硬；若继续 owner-shift 方向，下一步不再只是 judgment，而是必须把 widened subset 推到更完整一拍 execution subset，同时继续把 true exit / uplift absorbed / cleanup 后置。
- **Approver:** Room 1

### D-OPP-001-109
- **Decision:** Room 1 接受 `R1_to_R4_P3_3_13_Execution_Handoff_v0.1.md` 所定义的本轮 very narrow execution layer：只进入 ReviewPage + 首页 review 承接层的 widened execution subset、`review_group` true-exit-candidate、DB/API uplift-absorb-readiness 与 stronger-ingest absorb-readiness binding prep，不得把 full cutover、`review_group` true exit、active DB/API uplift absorbed 或 cleanup 写成当前事实。
- **Why:** Room 2 / Room 3 / Room 5 的专项输入已被统一收口为一份短而硬的执行单，足以支撑 Room 4 在不触碰 homepage route、active continuation、final fact owner、true exit、active uplift 与 cleanup bundling 的前提下完成本轮实施。
- **Approver:** Room 1

### D-OPP-001-110
- **Decision:** Room 1 将运行态 BR / UI 主文件入口更新为 `BR-OPP-001_v0.2.15.md` 与 `UI_SPEC_v0.3.5.md`，并在当前主线程中吸收 `P3.3.13` 为 **current-round closed**。
- **Why:** User 已明确告知 BR / UI 主文件完成回写，且已转述 Room 4 完成 `P3.3.13` 的开发 / 实施；而本轮 handoff 的完成定义本身只要求 execution / true-exit-candidate / uplift-absorb-readiness 的 very narrow subset 落地，不要求发生 full cutover completed、`review_group` true exit、active DB/API baseline uplift absorbed 或 cleanup。
- **Approver:** Room 1 / User relay

## 6) Gate Gaps (for MAIN reference)

### G-OPP-001-001
- **Gap:** `post-P3.3.13` 的下一轮产品 / 技术 focus 尚未被 Room 1 / User 正式拍板。
- **Impact:** 当前可接受 P3.3.13 close，但项目若不尽快 pin 新 focus，会再次进入“当前轮已关单、下一轮未起单”的推进真空。
- **Owner:** Room 1 / User
- **Priority:** Critical

### G-OPP-001-002
- **Gap:** 更深一层的 **更接近 full cutover 的 fuller-cutover / `review_group` true-exit-gate / DB-API uplift-absorb judgment** 仍保持 pending。
- **Impact:** P3.3.13 已把 widened subset 推到 execution subset 一小层；若项目继续深化，下一轮应单开更接近 full cutover 的 fuller-cutover / true-exit-gate / DB-API uplift-absorb judgment round，不可在当前 closeout 后静默把 `review_group` true exit、baseline uplift absorbed 与 cleanup 一并拉满。
...
- **Impact:** 当前 P3.3.13 允许 close，因为本轮明确未触碰 DB schema / API core semantics；但若下一轮继续 owner-shift 方向，Room 2 必须单开新的 DB / API uplift-absorb judgment / next-step execution round，避免治理层与运行态再次脱节。
- **Owner:** Room 2 / Room 1
- **Priority:** High
...
1. **Owner=Room 1 / User | ETA=Next round | Done=拍板 `post-P3.3.13` 的下一推进主题，并明确是继续进入“更接近 full cutover 的 fuller-cutover / true-exit-gate / DB-API uplift-absorb judgment”方向，还是切到别的产品主题 | Action=让项目从 “P3.3.13 已 close” 平滑切到下一推进主题**

2. **Owner=Room 2 | ETA=If owner-shift direction continues | Done=给出下一轮 `fuller-cutover absorb-candidate / review_group true-exit-gate / DB-API uplift-absorb judgment / migration / rollback` 的范围、红线与 staged rollout 建议 | Action=为可能的 P3.3.14 / next-step fuller-cutover judgment round 准备技术入口**
...
- 当前最重要的新事实是：**`P3.3.13` 已按 fuller-cutover execution / true-exit-candidate / uplift-absorb-readiness 的 very narrow subset 实施完成，并被 Room 1 吸收到主线程。**
- 本轮真正落地的是：继续把扩大方向压在 **ReviewPage + 首页 review 承接层** 的 execution subset，继续保留 `review_group` 作为 current owner + retained fallback anchor，并继续把 stronger-ingest、rollback / hold / stop-condition / observability 与 uplift-absorb-readiness seam families 写硬。
...
- 当前 active BR / UI baseline 已升级为 `BR-OPP-001_v0.2.15.md` 与 `UI_SPEC_v0.3.5.md`；DB / API baseline 继续保持 `v0.2.1`。
- 下一治理动作不是在 P3.3.13 内静默继续 owner shift，而是 **由 Room 1 / User 拍板 post-P3.3.13 下一轮主题；若继续此方向，则单开下一轮 fuller-cutover / true-exit-gate / DB-API uplift-absorb judgment round。**


## 9) Working Rule

> **Main 现在必须反映真实主线程：P1 / P2 / Option A / Option A.1 / Option B / Option C / P3 / P3.1 / P3.3 / P3.3.2 / P3.3.3 / P3.3.4 / P3.3.5 / P3.3.6 / P3.3.7 / P3.3.8 / P3.3.9 / P3.3.10 / P3.3.11 / P3.3.12 / P3.3.13 均已完成当前轮关单；其中 P3.3.6 只把 owner-shift 方向推进到 `Compatibility Contract v1 + Shadow-Entry Prep`，P3.3.7 只把该方向推进到 `Phase 2 / Limited Execution / Shadow Mode` 的 very narrow subset，P3.3.8 只把该方向推进到 `Phase 3 / Gate-Driven Candidate Execution + Migration Prep` 的 very narrow subset，P3.3.9 只把该方向推进到 `First Very Narrow Cutover` 的第一拍，P3.3.10 只把该方向推进到 `fuller cutover / review_group exit-gate / DB-API uplift judgment` 的 very narrow subset，而 P3.3.11 则只把该方向推进到 `fuller-cutover execution / review_group exit-candidate / DB-API uplift-readiness` 的 very narrow subset：继续只在 ReviewPage + 首页 review 承接层扩大一小层 execution-ready subset，继续保留 `review_group` 为 current owner + retained fallback anchor，继续守住首页 `study_default`、active continuation 独立承接、final fact / settlement 以后端为准、DB/API 不做 core rewrite 或 active uplift；而 P3.3.12 进一步只把该方向推进到 `fuller-cutover / true-exit-gate / DB-API uplift-absorb judgment` 的 very narrow subset：继续只在 ReviewPage + 首页 review 承接层形成 absorb-candidate judgment artifacts，继续保留 `review_group` 为 current owner + retained fallback anchor，继续守住首页 `study_default`、active continuation 独立承接、final fact / settlement 以后端为准、DB/API 不做 active uplift absorbed，`review_group` true exit / active DB-API uplift absorbed / cleanup 后置；而 P3.3.13 则进一步只把该方向推进到 `fuller-cutover execution / true-exit-candidate / DB-API uplift-absorb-readiness` 的 very narrow subset：继续只在 ReviewPage + 首页 review 承接层扩大一小层 execution subset，继续保留 `review_group` 为 current owner + retained fallback anchor，继续守住首页 `study_default`、active continuation 独立承接、final fact / settlement 以后端为准、DB/API 不做 active uplift absorbed，`review_group` true exit / cleanup 继续后置。下一步不是在 P3.3.13 内继续补脑，而是由 Room 1 / User 明确 pin `post-P3.3.13` 的下一推进主题；若继续 owner-shift 方向，则应单开下一轮 fuller-cutover / true-exit-gate / DB-API uplift-absorb judgment round。**
