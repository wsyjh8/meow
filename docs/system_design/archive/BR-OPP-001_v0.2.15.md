...
**Base merge:** `BR-OPP-001_v0.2.14.md` + `P3.3.13 closeout inputs`  
**Merge policy:** 保留 `v0.2.14` 的 full BR 结构，并将 P3.3.13 已收口的规则吸收为单文件可读的增量主文档候选
...
- `BR-OPP-001_v0.2.14.md`（latest BR review basis / patch base for this round）
...
> Note:
> 1. 本稿以上述 current review basis 为准，并以 `P3.3.13 — Fuller-Cutover Execution / True-Exit-Candidate / DB-API Uplift-Absorb-Readiness Round` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.14.md` 的 full BR 结构上，增量吸收 P3.3.13 已收口、且已被 Room 1 同意进入主 BR 回写的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.13 规则；fuller-cutover execution / true-exit-candidate / uplift-absorb-readiness 的结果仍不得被写成 runtime owner shift completed、ReviewPage local-serving full runtime cutover completed、`review_group` true exit、active DB/API baseline uplift absorbed、cleanup / old-path purge、homepage route / planner-aware runtime route、active continuation source switch、final fact owner shift、DB schema rewrite、API core semantics rewrite 等既成事实。
...
### 3.1P 已冻结（P3.3.13 closeout）
当前已足够跨文档一致、且可并入 full BR baseline 的 P3.3.13 收口规则有：
1. **`fuller_cutover_execution_subset_v2` 已冻结更完整一拍 execution subset 边界：当前只允许把 P3.3.12 的 widened family 从 judgment 再前进一步到 execution subset；允许进入的 widened family 仍只限于 `ReviewPage continuity-adjacent serving-adapter family`、与其强绑定的 source-neutral helper / summary / empty-state / completion 前置说明层、首页 review helper / summary / no-review-state 的 retained-anchor-aware contract、rollback / hold / fallback 的中性 copy / state contract，以及 stronger-ingest absorb-candidate binding prep**
2. **P3.3.13 当前明确禁止扩大到：首页默认主 route 切换、active continuation source switch、user-visible planner-aware route / auto-routing runtime、`review_group` true exit、final fact owner shift、active DB/API baseline uplift absorbed、以及 cleanup / old-path purge；widened execution subset 只表示“已具备进入更完整一拍 execution-preflight 的资格”，不表示当前已经完成 fuller cutover**
3. **`review_group_true_exit_candidate_v1` 已冻结最小 true-exit-candidate 边界：`review_group` 当前仍必须继续保持 `current runtime serving owner + retained fallback anchor + compatibility anchor + deprecated candidate`；当前只允许把 still-dependent paths inventory / replacement-readiness、retained-anchor future-narrowable 范围、fallback / rollback future-narrowable 范围、helper / summary / CTA / empty-state 脱离 group-only wording 的准备，以及 true-exit-gate 所需 contract / runtime / test / doc / fallback 条件推进到 true-exit-candidate 层**
4. **在 P3.3.13 当前轮，以下内容仍必须继续保持 current owner + retained fallback anchor：current visible owner 身份、rollback target、active continuation identity、completion gating、settlement trigger、non-cutover / non-upgraded sessions baseline path，以及 compatibility anchor / QA baseline reference；只要其中任一仍未被清晰替代，`review_group` 就不得被写成已可 true exit**
5. **`db_api_uplift_absorb_readiness_v1` 已冻结最小 readiness 边界：当前最多只允许 serving source descriptor seam、retained-anchor / fallback posture seam、stronger-ingest path minimal seam、rollback / hold / observability seam、以及 source-neutral state / helper / summary contract seam 进入 uplift-absorb-readiness；`review_group` true-exit、active continuation source switch、final fact owner shift、homepage route / planner-aware route、以及 DB schema rewrite / API core semantics rewrite 相关 seam 当前仍只能停留在 marker / migration / rollback / hold 层**
6. **`cutover_vs_fact_owner_boundary_v5` 已冻结当前红线：即使 fuller-cutover execution 再前进一步，有效复习事实、今日目标完成、奖励结算 / 账本到账、`check_in / learning_day / streak` 与 completion / 到账类主反馈当前仍必须以后端 / cloud fact layer 为准；local stronger-ingest path 最多只允许进入 absorb-readiness-level stronger candidate，不允许升格为 final fact owner**
7. **`true_exit_candidate_narrowing_guardrail_v1` 已冻结当前可缩窄范围：本轮只允许 very narrow 缩窄 source-neutral helper / summary wording 的 group-only 耦合、首页 review helper / empty-state / no-review-state 的 retained-anchor-aware 表达、rollback / fallback 说明中的历史性冗余 wording，以及 QA / docs 中哪些 UI 资产已不再必须 group-only 的判断；rollback target、active continuation identity、completion gating、settlement trigger、non-cutover baseline path 与 current visible owner 身份当前都不得缩窄**
8. **`phase7_writeback_order_v1` 已冻结最小治理顺序：当前仍必须先 Room 2 tech note，再 Room 3 rules note，再 Room 5 UI preflight，再由 Room 1 absorb / pin；当前只能把 widened execution subset、true-exit-candidate、uplift-absorb-readiness、以及 stronger-ingest absorb-readiness binding prep 写成 execution-ready candidate，不得写成 full cutover completed、`review_group` true exit、active DB/API uplift absorbed、final fact owner shift、或 cleanup 已生效**
9. **P3.3.13 当前继续明确：这是 fuller-cutover execution / true-exit-candidate / uplift-absorb-readiness round，不是 full cutover completed / true exit / uplift absorbed round；任何“已切到本地规划 / 本地已接管复习 / `review_group` 已退场 / 新主链路已生效 / cutover 已完成 / 本地结果已写回最终事实 / active DB/API baseline 已升级 / uplift 已 absorbed / true exit 已开始 / 现在已经可以清理旧 path”的表达，继续属于 overclaim 禁区**
...
32. widened execution subset v2 真正进入更完整一拍 Room 4 execution handoff 时，ReviewPage + 首页 review 承接层的最小执行顺序应先开哪一段
33. `review_group` 何时才允许从 true-exit-candidate 进入 true exit 审查前的 execution-prep，并开始缩窄 current owner / rollback target 之外的路径级依赖
34. uplift-absorb-readiness seam 何时才允许进入 active baseline uplift absorbed 审查前的 execution-preflight / absorbed-candidate 层
35. absorb-readiness-level stronger candidate 何时才允许进入更强的 active ingest path，而仍不构成 final fact owner shift
36. true-exit-candidate narrowing 何时才允许从 wording / helper 层进入 path-level narrowing，而仍不破坏 current runtime truth 与 rollback target
...
### BR-089 P3.3.13 `fuller_cutover_execution_subset_v2`
- **Status:** Frozen
- **Rule:** P3.3.13 当前正式冻结 `fuller_cutover_execution_subset_v2`：当前只允许把 P3.3.12 的 widened family 从 judgment 再前进一步到更完整一拍的 execution subset；允许进入的 widened family 仍只限于 `ReviewPage continuity-adjacent serving-adapter family`、与其强绑定的 source-neutral helper / summary / empty-state / completion 前置说明层、首页 review helper / summary / no-review-state 的 retained-anchor-aware contract、rollback / hold / fallback 的中性 copy / state contract，以及 stronger-ingest absorb-candidate binding prep。
- **Applies to:** BR / UI / DB / API / TEST / cutover governance
- **Checkable:**
  1. 当前不得把 widened execution subset 扩大到首页默认主 route、active continuation source switch、user-visible planner-aware route / auto-routing runtime、`review_group` true exit、final fact owner shift、active DB/API baseline uplift absorbed、或 cleanup / old-path purge
  2. execution subset 只表示“已具备进入更完整一拍 execution-preflight 的资格”，不表示 fuller cutover 已完成
  3. widened subset 仍必须守住最小 rollback 与最小 blast radius，不得静默放大到 ReviewPage + 首页 review 承接层之外

### BR-090 P3.3.13 `review_group_true_exit_candidate_v1`
- **Status:** Frozen
- **Rule:** P3.3.13 当前正式冻结 `review_group_true_exit_candidate_v1`：`review_group` 仍必须继续保持 `current runtime serving owner + retained fallback anchor + compatibility anchor + deprecated candidate`；当前只允许把 still-dependent paths inventory / replacement-readiness、retained-anchor future-narrowable 范围、fallback / rollback future-narrowable 范围、以及 helper / summary / CTA / empty-state 脱离 group-only wording 的准备推进到 true-exit-candidate 层。
- **Applies to:** BR / UI / DB / API / TEST / exit governance
- **Checkable:**
  1. 只要 active continuation identity、completion gating、settlement trigger、rollback target、non-cutover / non-upgraded sessions baseline path、或 compatibility anchor / QA baseline reference 仍未被清晰替代，`review_group` 就不得被写成已可 true exit
  2. current visible owner 身份、rollback target、active continuation identity、completion gating、settlement trigger 与 non-cutover baseline path 当前都必须继续保持 current owner + retained fallback anchor
  3. true-exit-candidate ≠ true exit；当前不得把任何 narrowing / replacement-ready 判断写成 true exit、fallback-only、historical-only、或可直接清理

### BR-091 P3.3.13 `db_api_uplift_absorb_readiness_v1`
- **Status:** Frozen
- **Rule:** P3.3.13 当前正式冻结 `db_api_uplift_absorb_readiness_v1`：当前最多只允许 serving source descriptor seam、retained-anchor / fallback posture seam、stronger-ingest path minimal seam、rollback / hold / observability seam、以及 source-neutral state / helper / summary contract seam 进入 uplift-absorb-readiness。
- **Applies to:** BR / DB / API / UI / TEST / baseline governance
- **Checkable:**
  1. `review_group` true-exit、active continuation source switch、final fact owner shift、homepage route / planner-aware route、以及 DB schema rewrite / API core semantics rewrite 相关 seam 当前仍只能停留在 marker / migration / rollback / hold 层
  2. uplift-absorb-readiness 不得写成 active DB/API baseline 已升级、endpoint meaning 已重写、或 runtime truth 已同步替换
  3. 当前不得借 uplift-absorb-readiness 宣称 active baseline 已 absorbed 或重写整个 DB/API 主文档

### BR-092 P3.3.13 `cutover_vs_fact_owner_boundary_v5`
- **Status:** Frozen
- **Rule:** P3.3.13 当前正式冻结 `cutover_vs_fact_owner_boundary_v5`：即使 fuller-cutover execution 再前进一步，有效复习事实、今日目标完成、奖励结算 / 账本到账、`check_in / learning_day / streak` 与 completion / 到账类主反馈当前仍必须以后端 / cloud fact layer 为准；local stronger-ingest path 最多只允许进入 absorb-readiness-level stronger candidate，不允许升格为 final fact owner。
- **Applies to:** BR / UI / DB / API / TEST / settlement governance
- **Checkable:**
  1. 当前不得把 local stronger-ingest absorb-readiness candidate 写成上述最终事实已可由本地裁定
  2. stronger-ingest candidate 当前最多只允许进入更清楚的 accept / reject / duplicate / progress-candidate / completion-candidate 规则、更明确的 precondition / postcondition / hold-reason / evidence ownership、以及与 widened execution subset 直接绑定的最小 ingest contract
  3. `本地已接管复习事实 / 今日完成判定 / 奖励到账 / streak主导 / completion由本地裁定 / review_group已退场 / active DBAPI已升级 / uplift已absorbed / fuller cutover已完成 / 新主链路已生效` 继续属于 overclaim

### BR-093 P3.3.13 `true_exit_candidate_narrowing_guardrail_v1`
- **Status:** Frozen
- **Rule:** P3.3.13 当前正式冻结 `true_exit_candidate_narrowing_guardrail_v1`：当前只允许 very narrow 缩窄 source-neutral helper / summary wording 的 group-only 耦合、首页 review helper / empty-state / no-review-state 的 retained-anchor-aware 表达、rollback / fallback 说明中的历史性冗余 wording，以及 QA / docs 中哪些 UI 资产已不再必须 group-only 的判断；rollback target 仍必须固定为 `cloud_review_group_current_runtime_path`。
- **Applies to:** BR / UI / DB / API / TEST / rollback governance
- **Checkable:**
  1. rollback target、active continuation identity、completion gating、settlement trigger、non-cutover baseline path 与 current visible owner 身份当前都不得缩窄
  2. 当前只允许“先缩表述和说明层，再缩路径级骨架”，不得反向推进
  3. 任何把 true-exit-candidate narrowing 写成 current owner posture 已可改变、rollback target 已可替换、或 `review_group` 已可清理的表达都属于越界

### BR-094 P3.3.13 `phase7_writeback_order_v1`
- **Status:** Frozen
- **Rule:** P3.3.13 当前正式冻结 `phase7_writeback_order_v1`：当前仍必须先 Room 2 tech note，再 Room 3 rules note，再 Room 5 UI preflight，再由 Room 1 absorb / pin；当前只能把 widened execution subset、true-exit-candidate、uplift-absorb-readiness、以及 stronger-ingest absorb-readiness binding prep 写成 execution-ready candidate，不得写成 full cutover completed、`review_group` true exit、active DB/API uplift absorbed、final fact owner shift、或 cleanup 已生效。
- **Applies to:** BR / DB / API / UI / TEST / rollout governance
- **Checkable:**
  1. 当前若要被 Room 1 吸收到下一轮 `R1 → R4` handoff，至少必须满足 no-overclaim、rollback / hold / fallback / observability 已成套、不触碰首页 route / active continuation / final fact owner、`review_group` 仍保持 current owner + retained anchor、且不把 uplift-absorb-readiness 写成 active uplift
  2. execution-ready candidate / true-exit-candidate / uplift-absorb-readiness / runtime truth 必须继续分层，禁止静默混写
  3. 当前不得把更完整一拍 execution subset 误写成 full cutover 已完成或 true exit / uplift absorbed 已生效
...
### E-097 fuller-cutover execution subset v2 被误写成 full cutover completed
- **Expected:** 禁止；当前 widened family 只进入更完整一拍 execution subset / execution-preflight，不得被写成 runtime 主链路已经整体切换

### E-098 `review_group` true-exit-candidate 被误写成 true exit / fallback-only / 可清理
- **Expected:** 禁止；当前仍必须保持 current runtime owner + retained fallback anchor + compatibility anchor + deprecated candidate，不得因为 candidate / narrowing 讨论就宣告 true exit 或 cleanup

### E-099 uplift-absorb-readiness 被误写成 active DB/API baseline uplift absorbed
- **Expected:** 禁止；当前只到 readiness 层，不得写成 active baseline 已升级、endpoint meaning 已改写、或 runtime truth 已同步替换

### E-100 absorb-readiness-level stronger-ingest 被误写成 final fact owner 已前进
- **Expected:** 禁止；serving seam advancement 仍不等于 final fact owner advancement，本地 stronger-ingest 不得直接改 ledger / daily goal / streak / settlement / completion 最终事实

### E-101 true-exit-candidate narrowing 被误写成 rollback target / current owner posture 已可缩窄
- **Expected:** 禁止；当前允许缩窄的只限 wording / helper / empty-state / docs 级依赖，rollback target、current visible owner、active continuation、completion gating、settlement trigger 与 non-cutover baseline path 仍必须固定

### E-102 execution-ready / true-exit-candidate / uplift-absorb-readiness / runtime truth 四层被静默混写
- **Expected:** 禁止；任何 write-back、UI、测试或 closeout 若跳过层级、把 readiness / candidate 直接说成 truth，都视为 silent contract drift
...
### 6.1J P3.3.13 术语补充
1. `fuller_cutover_execution_subset_v2`：比 P3.3.12 的 absorb-candidate judgment 更前进一步的 widened execution subset；当前仍主要局限于 ReviewPage + 首页 review 承接层，不等于 full cutover completed
2. `true-exit-candidate`：`review_group` 从 true-exit-gate judgment 再前进一步的候选层；只说明“更接近 true exit 的资格判断”，不说明当前已 true exit
3. `uplift-absorb-readiness`：比 uplift-absorb judgment 更接近 active baseline uplift absorbed 的 readiness 层；不等于 active DB/API baseline 已升级
4. `absorb-readiness-level stronger candidate`：比 absorb-judgment-level stronger candidate 更前进一步的 stronger-ingest 候选；当前仍不是 final fact owner，也不自动构成 settlement owner shift
5. `true-exit-candidate narrowing`：只允许 very narrow 地缩窄 wording / helper / empty-state / docs 级的 retained-anchor 依赖；不包括 rollback target、current owner posture 或路径级骨架
6. `widened execution family`：P3.3.13 当前允许进入更完整一拍 execution subset 的 family，总体仍限定在 ReviewPage continuity-adjacent serving-adapter family + 首页 review 承接层
7. `execution subset / candidate / readiness / runtime truth`：P3.3.13 必须继续显式分开的四层；任何静默跨层升格都视为 overclaim 或 silent contract drift
...
### PD-036 widened execution subset v2 何时允许进入更完整一拍 Room 4 execution handoff
- **Question:** 当 P3.3.13 的 widened execution subset 已进入更完整一拍 execution-preflight 后，ReviewPage + 首页 review 承接层的最小执行顺序到底应先开哪一段？
- **Current state:** Pending
- **Room 3 default:** 当前只冻结 widened execution subset v2 的边界，不冻结真实执行顺序；是否进入更完整一拍 `R1 → R4` execution handoff，必须由 Room 1 结合 rollback / hold / observability 证据再次 pin。

### PD-037 `review_group` 何时允许从 true-exit-candidate 进入 true exit 审查前的 execution-prep
- **Question:** `review_group` 何时才允许从 true-exit-candidate 进入更进一步的 true exit 审查 / execution-prep，并开始缩窄 current owner / rollback target 之外的路径级依赖？
- **Current state:** Pending
- **Room 3 default:** 当前只冻结 true-exit-candidate 与 narrowing guardrails，不冻结 true exit 审查前的执行时点；只要 rollback target、current visible owner、active continuation、completion gating、settlement trigger 仍未有清晰替代，均不得进入 true exit execution-prep。

### PD-038 uplift-absorb-readiness seam 何时允许进入 active baseline uplift absorbed 审查前的 execution-prep
- **Question:** uplift-absorb-readiness seam families 何时才允许进入 active baseline uplift absorbed 审查前的 execution-prep / absorbed-candidate 层，而不被误写成 active baseline 已升级？
- **Current state:** Pending
- **Room 3 default:** 当前只冻结 readiness 层，不冻结 absorbed 审查前的 execution-prep；一旦涉及 active DB/API baseline uplift absorbed、DB schema rewrite 或 API core semantics rewrite，必须先升级给 Room 2 / Room 1 再单独 scope pin。

## 10. 变更记录

### v0.2.15 (2026-04-11)
- 以 `BR-OPP-001_v0.2.14.md` 为 base，吸收 P3.3.13 已收口、且已被 Room 1 同意进入主 BR 回写的规则边界，形成 `P3.3.13 closeout` 的 BR 增量主文档候选
- 同步输入依据到 `R1_P3_3_13_ScopePin_and_Handoff_Pack_v0.1.md`、`R2_P3_3_13_FullerCutoverExecution_TrueExitCandidate_and_DBUpliftAbsorbReadiness_Tech_Note_v0.1.md`、`R3_P3_3_13_FullerCutoverExecution_TrueExitCandidate_and_DBUpliftAbsorbReadiness_Rules_Note_v0.1.md`、`UI_SPEC_P3_3_13_FullerCutoverExecution_TrueExitCandidate_and_DBUpliftAbsorbReadiness_UI_Preflight_v0.1.md`、`Main_updated_2026-04-10_v33.md`、`STATUS_updated_2026-04-10_v31.md`
- 新增 BR-089 ~ BR-094：`fuller_cutover_execution_subset_v2`、`review_group_true_exit_candidate_v1`、`db_api_uplift_absorb_readiness_v1`、`cutover_vs_fact_owner_boundary_v5`、`true_exit_candidate_narrowing_guardrail_v1`、`phase7_writeback_order_v1`
- 更新 E-097 ~ E-102，补齐 widened execution subset 被误写成 full cutover completed、`review_group` true-exit-candidate 被误写成 true exit / cleanup、uplift-absorb-readiness 被误写成 active baseline uplift absorbed、absorb-readiness-level stronger-ingest 被误写成 final fact owner 已前进、true-exit-candidate narrowing 被误写成 rollback target / current owner posture 已可缩窄、以及 execution-ready / candidate / readiness / runtime truth 四层被静默混写的高风险边界用例
- 新增 6.1J glossary 补充，并新增 PD-036 ~ PD-038，保留 widened execution subset v2 的真实执行顺序、`review_group` 从 true-exit-candidate 进入 true exit 审查前 execution-prep 的时点、以及 uplift-absorb-readiness seam 进入 active baseline uplift absorbed 审查前 execution-prep 的时点为 pending
...
- **This file:** `BR-OPP-001_v0.2.15.md`
- **Owner:** Room 3
...
- **Runtime note:** 本稿以 `BR-OPP-001_v0.2.14.md` 为 base，并吸收 P3.3.13 已收口的 `fuller_cutover_execution_subset_v2`、`review_group_true_exit_candidate_v1`、`db_api_uplift_absorb_readiness_v1`、`cutover_vs_fact_owner_boundary_v5`、`true_exit_candidate_narrowing_guardrail_v1` 与 `phase7_writeback_order_v1` 的最小合同；**current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **P3.3.13 note:** 本版继续在 P3.3 / P3.3.1 / P3.3.2 / P3.3.3 / P3.3.4 / P3.3.5 / P3.3.6 / P3.3.7 / P3.3.8 / P3.3.9 / P3.3.10 / P3.3.11 / P3.3.12 基础上，吸收 P3.3.13 已收口的规则：fuller-cutover 当前只允许把 widened family 从 judgment 再前进一步到 execution subset；`review_group` 当前只进入 true-exit-candidate，继续保持 current runtime owner + retained fallback anchor + compatibility anchor + deprecated candidate；DB/API 当前只进入 uplift-absorb-readiness seam families；absorb-readiness-level stronger candidate 当前仍不越过 backend final fact owner 边界；full cutover completed、`review_group` true exit、active DB/API baseline uplift absorbed、cleanup / old-path purge、homepage route switch、active continuation source switch、auto-routing runtime、final fact owner shift 与任何用户可见 cutover / exit / uplift 宣告继续保持 pending。
- **Maintenance rule:**
