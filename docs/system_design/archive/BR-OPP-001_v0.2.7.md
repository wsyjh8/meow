...
**Base merge:** `BR-OPP-001_v0.2.6.md` + `P3.3.5 closeout inputs`  
**Merge policy:** 保留 `v0.2.5` 的 full BR 结构，并将 P3.3.4 已收口的规则吸收为单文件可读的增量主文档候选
...
- `BR-OPP-001_v0.2.6.md`（current runtime active BR baseline）
...
> Note:
> 1. 本稿以上述 current active runtime basis 为准，并以 `P3.3.5 — Local Planner Owner Shift / Cloud Backup Rebase Round` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.6.md` 的 full BR 结构上，增量吸收 P3.3.5 已收口、且已被 Room 1 pin 为最小合同的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.5 规则；runtime owner shift completed、ReviewPage local-serving cutover、auto-routing runtime、unified planner / planner merge、preview / explanation system 全量升级、full sync / real-time sync / auto merge 等继续保持 pending。
...
- **This file:** `BR-OPP-001_v0.2.7.md`
- **Owner:** Room 3
- **Suggested next state after review:** active after Room 1 runtime-baseline update（当前运行态 active BR baseline 仍为 `BR-OPP-001_v0.2.3.md`，待 Main / STATUS 更新）
- **Runtime note:** 本稿以上一版 candidate BR `BR-OPP-001_v0.2.6.md` 为 base，并吸收 P3.3.5 已收口的 `planner_owner_shift_v2`、`review_serving_contract_v2`、`session_entry_and_routing_v2`、`preview_and_explanation_contract_v2`、`backup_restore_and_cross_device_boundary_v2` 与 `migration_and_deprecation_plan_v1` 的最小合同；**current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **P3.3.5 note:** 本版继续在 P3.3 / P3.3.1 / P3.3.2 / P3.3.3 / P3.3.4 基础上，吸收 P3.3.5 已收口的规则：future local primary planning owner 方向被接受，但只停在 target-state candidate；ReviewPage current serving truth 继续保持 cloud `review_group`，同时进入 compatibility / deprecation path；backup / restore / cross-device 三层语义继续严格分离，并保持 manual-only / no real-time sync / no auto merge；runtime owner shift completed、local-serving cutover、planner merge / unified planner、preview / explanation system 全量升级与 full sync 等继续保持 pending。
- **Maintenance rule:**
