# BR-OPP-001_v0.1.9 Full
**Project:** 背单词喵喵 App  
**Owner:** Room 3  
**Type:** Business Rules / Governance SSOT Candidate / full merged baseline  
**Status:** full merged BR baseline candidate / ready for Room 1 review  
**Version:** v0.1.9-full  
**Last updated:** 2026-04-08
**Base merge:** `BR-OPP-001_v0.1.8.md` + `BR-OPP-001_v0.1.9.md`
**Merge policy:** 保留 `v0.1.8` 的 full BR 结构，并将 `v0.1.9` 的 P3.1 direct-scope delta 规则吸收为单文件可读的完整基线
...
- `UI_SPEC_P3_1_LocalProgress_CloudBackup_v0.1.1.md`（retained P3.1 base UI baseline reference）
- `UI_SPEC_P3_1_DirectScopePin_Delta_v0.1.1.md`（current P3.1 delta UI reference）
- `UI_SPEC_OptionC_MainMechanism_v0.1.1.md`（retained historical main-mechanism active baseline reference）
- `UI_SPEC_v0.1.4.md`（retained Option A / persistence guardrail reference）
...
- `BR-OPP-001_v0.1.7.md`（current runtime active BR baseline）
...
- `R1_P3_Formal_Direction_Pin_and_Handoff_Pack_v0.1.md`
- `R2_P3_MainMechanism_Deepening_Preflight_v0.1.1.md`
- `R3_P3_Rules_Freeze_Note_v0.1.1.md`
- `R1_P3_1_DirectScopePin_3Buttons_Handoff_Pack_v0.1.md`
...
- `BR-OPP-001_v0.1.9.md`（patch-style direct-scope delta write-back candidate）

> Note:
> 1. 本稿以上述 current active runtime basis 为准，并以 `P3 rules freeze` + `P3.1 direct-scope delta` 两轮输入合并收口。
> 2. 本稿目标不是保留“`v0.1.8` + `v0.1.9 patch` 双文件阅读”，而是把两轮已收敛规则合并成单文件 full BR baseline candidate。
> 3. `UI_SPEC_OptionC_MainMechanism_v0.1.1.md` 仍是历史主机制 active UI baseline；`UI_SPEC_P3_1_DirectScopePin_Delta_v0.1.1.md` 作为当前 P3.1 delta UI 参考输入，但不自动替代推进层 pin。
> 4. 当前 runtime active baseline 仍以后续被 Main / STATUS pin 的版本为准；本稿只是供 Room 1 review / pin 的 full merged BR candidate。
...
### 9.1 给 Room 1
1. 将本文件作为 `BR-OPP-001_v0.1.8.md` 的增量升版候选，供下一轮治理层 SSOT 吸收 / pin
2. 在 Main / Status 中吸收以下新增已冻结结论：
   - CTA winner 从 Option C 最小仲裁层级推进到 P3 的更完整优先级算法边界
   - `review_group` 从最小合同推进到 P3 structured deepening 的 grouping / readiness / progress / priority 边界
   - statistics 从 `summary-first` 推进到“独立 minimal page / deeper minimal product 判断”边界，但默认 fallback 仍为 `summary-first`
   - “学习天数 = learning_day”继续保持为 frozen 统计口径
   - `streak` 进入 future basis 的正式决策准备边界，但当前 runtime truth 仍保持 `check_in`
3. 若 Room 1 后续还要接受 contract deepening，应明确 pin：
   - `today_primary_action` 是否进入 active contract
   - 更细 `review_summary` / `next_group_readiness` / `progress_summary` 是否补 contract deepening
   - statistics 是否从 summary-first 正式展开为独立 minimal page
   - future streak decision-support / migration-support fields 是否进入 active contract
4. 若 Room 1 正式 pin Option A，应继续保留并在实现窗口执行 post-P2 persistence guardrails：
   - 同源一致性
   - 只读 / maintenance / temporarily_unavailable 的最小 user-facing 语义
   - `displayed snapshot ≠ fresh backend truth ≠ success`

### 9.2 给 Room 2
1. 基于本文件继续保持 DB / API / plan / TEST 的 persistence sync
2. 至少完成：
   - 切流窗口不混源
   - Today / Secondary / Customize 的 parity / regression 验收
   - maintenance / read_only / temporarily_unavailable 的 contract 明确
   - 降级窗口内不出现假成功或 generic error 吞语义

### 9.3 给 Room 5
1. 基于本文件提交 P3 / P3.1 的统一 UI / UX 收口；direct-scope delta 中必须继续守住 upload / download / restore 三层语义、restore warning、`daily_goal` 不回溯历史、以及 destructive actions 不进本轮
2. 至少完成：
   - Today 页仍只有一个最强主 CTA
   - active `review_group` 存在时，默认先承接“继续本组复习”
   - 更细 CTA / review deeper states 只能在 Room 1 pin 相关 contract 后进入；未 pin 前不得本地补脑
   - statistics 在 Room 1 未 pin独立 minimal page 前，只能按 `summary-first` 落地；“学习天数”必须基于 `learning_day`
   - `streak` 当前仍按签到延续；future basis 只能写成决策准备 / migration explanation，不得写成当前已切 basis
   - 签到成功 ≠ learning day
   - 在 Room 1 未 pin P3 UI 输入前，Room 5 只将本文件作为 P3 UI 收口依据，不自动替代当前 runtime active UI baseline

### 9.4 给 Room 4
1. 把 BR-009 / BR-010 / BR-011 / BR-012 / BR-014 / BR-015 / BR-007 / BR-017 / BR-018 / BR-019，以及新增的 BR-020 / BR-021 / BR-022 / BR-023 / BR-024 / BR-025 纳入最小回归集
2. 测试至少覆盖：
   - active `review_group` 存在时 CTA 默认先承接 continuation
   - 无 active group 但后端确认高优先复习任务时，允许“先去复习”胜出
   - Session 在 P3 更完整优先级算法阶段仍不是自动最高优先级，除非 Room 1 后续单独 pin
   - 本组完成 ≠ 今日复习完成
   - 同组跨 Session 继续
   - 同组重试不重复推进 / 不重复发奖
   - 未 pin `review_summary` / `next_group_readiness` / `progress_summary` contract 时，前端不得本地推断 readiness
   - statistics summary 中“学习天数 = learning_day”
   - 未 pin 独立 minimal page 前，statistics 默认回退到 `summary-first`
   - 签到成功但 `learning_day` 不成立
   - `learning_day` 成立但 `streak` 不延续
   - 未 pin basis switch 前，`streak_basis_type = check_in` 继续作为 current runtime truth
   - `local_date` 跨天边界
   - Today / Secondary / Customize 关键链路不混源
   - maintenance / read_only / temporarily_unavailable 的 user-facing 降级
   - `displayed snapshot ≠ fresh backend truth ≠ success`
...
- **This file:** `BR-OPP-001_v0.1.9_full.2.md`
- **Owner:** Room 3
- **Suggested next state after review:** active after Room 1 runtime-baseline update（当前运行态 active BR baseline 仍为 `BR-OPP-001_v0.1.7.md`，待 Main / STATUS 更新）
- **Runtime note:** This file uses the current runtime active baseline plus the converged P3 freeze note as review basis; **current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **Conditional guardrail note:** BR-017 / BR-018 / BR-019 属于 **conditional frozen guardrails**；它们只有在 Room 1 正式 pin Option A，并进入 migration / cutover / degraded-state 实施窗口后，才转为实现与测试强断言。
- **P3 / P3.1 note:** 本版已把 Room 1 已接受收敛的 P3 规则边界 + P3.1 direct-scope delta 已 close 的规则统一吸收进单文件 full BR；未被本版明确写入 BR 的完整算法、完整 contract、full sync / merge、独立 minimal page 正式进入、future basis switch、或 destructive actions rollout，不应视为 frozen。
- **Maintenance rule:**
  1. 未来凡涉及状态机、奖励触发、完成口径、文案边界、命名主口径的结论变更，必须更新本文件
