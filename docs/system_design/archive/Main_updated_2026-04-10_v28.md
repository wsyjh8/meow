...
- **Incremental update note:** 本版以 `Main_updated_2026-04-10_v26.md` 为 base，仅增量吸收 `P3.3.6 closeout`、`P3.3.7 closeout` 与 `BR / UI v0.2.8` runtime-baseline update；未被本轮触发的既有主线程内容保持不动。
...
面向**容易半途放弃背单词、但对可爱陪伴与轻养成有偏好的学习用户**，在**日常背词、复习、签到和碎片时间学习**场景下，解决**难开始、难坚持、反馈弱、长期留存低**的问题，通过**学习主机制 + 喵喵副机制承接**，带来**更高的打开率、完成率、复习率与长期陪伴式学习体验**；当前 `P3.1 — Local Progress + Cloud Backup` 的代码与 BR / DB / API / UI 文档已完成本轮对齐，Room 1 已将 runtime baseline 从旧 `v0.1.x` 正式提升到 `v0.2.1`；在此基础上，`P3.3` 已整体关单，`P3.3.2`、`P3.3.3`、`P3.3.4` 已分别把复习规划、preview 与 stronger bridge 推进到当前 very narrow minimal-contract 层。进一步地，`P3.3.5` 已完成 **Phase 0 / Compatibility-Prep**，`P3.3.6` 已完成 **Compatibility Contract v1 / Shadow-Entry Prep**，而 `P3.3.7` 本轮则把 owner-shift 方向进一步推进到 **Phase 2 / Limited Execution / Shadow Mode**：local-serving candidate、fact/settlement ingest candidate、routing shadow candidate 已开始在 dev / flag / QA evidence 层真实跑起来，并与 current cloud `review_group` 路径做 parity 对照；但 current runtime truth 继续保持 cloud review-serving layer，首页继续保持 `study_default`，final fact / settlement truth 继续以后端为准，`review_group` 当前仍是 current runtime owner + shadow baseline，而不是已被 local 接管或已退场。当前 Room 1 可将 `P3.3.7` 判定为 **current-round closed**，并将 active BR / UI runtime baselines 正式升级到 `BR-OPP-001_v0.2.8.md` 与 `UI_SPEC_v0.2.8.md`。
...
### G-OPP-001-001
- **Gap:** `post-P3.3.7` 的下一轮产品 / 技术 focus 尚未被 Room 1 / User 正式拍板。
- **Impact:** 当前可接受 P3.3.7 close，但项目若不尽快 pin 新 focus，会再次进入“当前轮已关单、下一轮未起单”的推进真空。
- **Owner:** Room 1 / User
- **Priority:** Critical
...
### G-OPP-001-003
- **Gap:** DB / API active baselines 仍停留在 `v0.2.1`，尚未进入与 P3.3.6 / P3.3.7 的 compatibility / ingest / shadow evidence 对齐的下一层候选吸收。
- **Impact:** 当前 P3.3.7 允许 close，因为本轮明确未触碰 DB schema / API core semantics；但若下一轮继续 owner-shift 方向，Room 2 必须单开新的 DB / API candidate round，避免治理层与运行态再次脱节。
- **Owner:** Room 2 / Room 1
- **Priority:** High
...
1. **Owner=Room 1 / User | ETA=Next round | Done=拍板 `post-P3.3.7` 的下一推进主题，并明确是继续进入 owner-shift Phase 3 gate / cutover-decision round，还是切到别的产品主题 | Action=让项目从 “P3.3.7 已 close” 平滑切到下一推进主题**

2. **Owner=Room 2 | ETA=If owner-shift direction continues | Done=给出下一轮 `DB / API compatibility + ingest + migration + phase-3-gate evidence` 的范围、红线与 staged rollout 建议 | Action=为可能的 P3.3.8 / next-step cutover-decision round 准备技术入口**
...
- 当前最重要的新事实是：**`P3.3.6` 与 `P3.3.7` 已分别按 Compatibility Contract v1 / Shadow-Entry Prep 和 Phase 2 / Limited Execution / Shadow Mode 范围实施完成，并被 Room 1 吸收到主线程。**
- 本轮真正落地的是：current runtime truth 继续不变，但 local-serving candidate、fact/settlement ingest candidate 与 routing shadow candidate 已在 dev / flag / QA evidence 层真实跑起来，并形成 parity / mismatch / stop-condition 证据集。
- `local primary planner owner` 当前仍只是 **future target-state candidate**；ReviewPage current serving truth 仍围绕 cloud `review_group`，首页仍保持 `study_default`，final fact / settlement truth 继续以后端为准。
- 当前 active BR / UI baseline 已升级为 `BR-OPP-001_v0.2.8.md` 与 `UI_SPEC_v0.2.8.md`；DB / API baseline 继续保持 `v0.2.1`。
- 下一治理动作不是在 P3.3.7 内静默继续 owner shift，而是 **由 Room 1 / User 拍板 post-P3.3.7 下一轮主题；若继续此方向，则单开下一轮 Phase 3 gate / cutover-decision + DB/API candidate round。**

## 9) Working Rule

> **Main 现在必须反映真实主线程：P1 / P2 / Option A / Option A.1 / Option B / Option C / P3 / P3.1 / P3.3 / P3.3.2 / P3.3.3 / P3.3.4 / P3.3.5 / P3.3.6 / P3.3.7 均已完成当前轮关单；其中 P3.3.6 只把 owner-shift 方向推进到 `Compatibility Contract v1 + Shadow-Entry Prep`，P3.3.7 只把该方向推进到 `Phase 2 / Limited Execution / Shadow Mode` 的 very narrow subset，未把 runtime owner shift、local-serving cutover、`review_group` 退场、auto-routing、planner merge、unified planner 或 full sync 写成当前事实。下一步不是在 P3.3.7 内继续补脑，而是由 Room 1 / User 明确 pin `post-P3.3.7` 的下一推进主题；若继续 owner-shift 方向，则必须单开下一轮 Phase 3 gate / cutover-decision + DB/API candidate round。**
