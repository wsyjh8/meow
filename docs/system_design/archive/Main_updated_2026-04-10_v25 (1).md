...
- **Incremental update note:** 本版以 `Main_updated_2026-04-10_v24.md` 为 base，仅增量吸收 `P3.3.4 closeout`；未被本轮触发的既有主线程内容保持不动。
...
- **Links / Entrances**
  ...
  - Room 4: `ROOM04_治理版_v0.2`
  - Room 5: `ROOM05_ROLE_CARD_UI_UX_v0.2.1`
  - BR: `BR-OPP-001_v0.2.5.md`
  - DB: `背单词喵喵app_DB设计草案_v0.2.1.md`
  - API: `背单词喵喵app_API设计草案_v0.2.1.md`
  - UI: `UI_SPEC_v0.2.5.md`

## 1) One-liner

面向**容易半途放弃背单词、但对可爱陪伴与轻养成有偏好的学习用户**，在**日常背词、复习、签到和碎片时间学习**场景下，解决**难开始、难坚持、反馈弱、长期留存低**的问题，通过**学习主机制 + 喵喵副机制承接**，带来**更高的打开率、完成率、复习率与长期陪伴式学习体验**；当前 `P3.1 — Local Progress + Cloud Backup` 的代码与 BR / DB / API / UI 文档已完成本轮对齐，Room 1 已将 runtime baseline 从旧 `v0.1.x` 正式提升到 `v0.2.1`；在此基础上，`P3.3` 已整体关单，而 `P3.3.2` 又把复习规划继续深化收口到 **minimal contract**：`session_entry_policy_v1` 已冻结为 `home_word_entry = study_default`，active `review_group` continuation 继续高优先但只能独立承接、不得 silent reroute；`planner_owner_split_v1` 已冻结为 **ReviewPage 由 cloud `review_group` 承担 serving truth owner，本地 FSRS 承担 device-side scheduling owner，复习提交流程继续 `cloud-first + local side-effect`**；当前 Room 1 可将 `P3.3.2` 判定为 **current-round closed**。在此基础上，`P3.3.3` 已把复习规划推进到 very narrow minimal contract；而 `P3.3.4` 又进一步把两条窄合同落到实现层：`previewDurations` 允许以 **local FSRS preview candidate** 的 estimated hint 形式回归，但当前只允许 **StudyPage only**、必须带“预计 / 仅供参考”语气、且不得进入 ReviewPage / 首页或升格为稳定计划事实；与此同时，ReviewPage bridge 允许从 `controlled best-effort` 收紧到 **stronger-but-still-non-blocking** 的最小技术合同（更强的 ensure / init、observability、failure handling floor、minimal repair path），但仍不得改变 cloud truth owner、不得回滚 cloud submit、不得进入 planner merge / unified planner；当前 Room 1 可将 `P3.3.4` 判定为 **current-round closed**。
...
### G-OPP-001-001
- **Gap:** `post-P3.3.4` 的下一轮产品 / 技术 focus 尚未被 Room 1 / User 正式拍板。
- **Impact:** 当前可接受 P3.3.4 close，但项目若不尽快 pin 新 focus，会再次进入“当前轮已关单、下一轮未起单”的推进真空。
- **Owner:** Room 1 / User
- **Priority:** Critical
...
1. **Owner=Room 3 / Room 5 | ETA=Next round | Done=将 `P3.3.4` 的已收口事实回写进新的 BR / UI 主文档候选（preview StudyPage-only estimated hint + stronger bridge 最小合同），并明确仍 pending 的 deeper planner 项 | Action=让治理层主文档追平当前实现现实**

2. **Owner=Room 1 / User | ETA=After BR/UI write-back or next planning round | Done=拍板 `post-P3.3.4` 的下一轮 focus，并明确是继续深化 preview / bridge、转 planner deeper-contract，还是切到别的产品主题 | Action=让项目从 “P3.3.4 已 close” 平滑切到下一推进主题**

3. **Owner=Room 1 | ETA=After new BR/UI candidate arrives | Done=视 write-back 结果决定是否升级 runtime active BR / UI baseline，并同步更新 STATUS / Main 的 active versions | Action=保持推进层与治理层 SSOT 同轨**

## 8) Notes (≤5 lines)

- 当前最重要的新事实是：**`P3.3.4` 已按 very narrow minimal contract 实施完成，并被 Room 1 吸收到主线程。**
- 本轮真正落地的是：StudyPage only 的 preview estimated hint，以及 stronger-but-still-non-blocking 的 ReviewPage bridge。
- ReviewPage / 首页继续不显示 preview；preview 仍不是计划事实，也不进入 active API / DB contract。
- 当前 active BR / UI baseline 继续保持 `BR-OPP-001_v0.2.5.md` 与 `UI_SPEC_v0.2.5.md`，等待 P3.3.4 主文档 write-back 后再决定是否升级。
- 下一治理动作不是继续在 P3.3.4 内补脑，而是 **补 BR / UI 主文档回写，并拍板 post-P3.3.4 下一轮 focus。**

## 9) Working Rule

> **Main 现在必须反映真实主线程：P1 / P2 / Option A / Option A.1 / Option B / Option C / P3 / P3.1 / P3.3 / P3.3.2 / P3.3.3 / P3.3.4 均已完成当前轮关单；其中 P3.3.4 只把 preview 推进到 StudyPage-only 的 estimated hint，并把 ReviewPage bridge 收紧到 stronger-but-still-non-blocking 的最小技术合同，未把 ReviewPage preview、planner merge、unified planner、或完整复习规划产品做厚。下一步不是继续在 P3.3.4 内补脑，而是补 BR / UI 主文档回写，并由 Room 1 / User 明确 pin `post-P3.3.4` 的下一推进主题。**