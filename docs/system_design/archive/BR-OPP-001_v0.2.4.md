...
**Base merge:** `BR-OPP-001_v0.2.3.md` + `P3.3.2 closeout inputs`  
**Merge policy:** 保留 `v0.2.2` 的 full BR 结构，并将 P3.3.2 已收口的规则吸收为单文件可读的增量主文档候选
...
- `BR-OPP-001_v0.2.2.md`（current runtime active BR baseline）
...
> Note:
> 1. 本稿以上述 current active runtime basis 为准，并以 `P3.3.2 — Review Planning Deepening / Contract Gate` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.3.md` 的 full BR 结构上，增量吸收 P3.3.2 已收口的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.2 规则；完整 SRS / auto-routing / unified planner / `previewDurations` future re-entry 等继续保持 pending。
...
9. `previewDurations` / interval preview 是否在未来进入 active contract
...
### 9.1 给 Room 1
1. 将本文件作为 `BR-OPP-001_v0.2.3.md` 的增量升版候选，供下一轮治理层 SSOT 吸收 / pin
2. 在 Main / Status 中吸收以下新增已冻结结论：
   - 4 按钮 final wording 已冻结为 `不认识 / 模糊 / 记得 / 秒答`
   - final wording 继续只属于 `rating input`，不得写成 mastery / result fact
   - ReviewPage bridge 已收口到 `controlled best-effort`
   - `previewDurations` 当前轮已正式 deferred，不进入 active contract
   - P3.3.1 仍不是主契约扩张，不得借收尾补强改 DB / API / `review_group` / planner owner
   - `session_entry_policy_v1` 已冻结：`home_word_entry = study_default`；active `review_group` continuation 高优先，但不等于 silent reroute
   - `planner_owner_split_v1` 已冻结：ReviewPage 继续 `cloud-first + local side-effect`；cloud `review_group` = serving truth owner，local FSRS = device-side scheduling owner
3. 继续保持 pending / future-focus item 不被偷升格：
   - mixed / auto-routing deeper session contract
   - 云端 `review_group` 与本地 FSRS 的最终 planner merge / unified planner 收敛方向
   - `StudyPage` / `ReviewPage` 是否长期统一
   - `previewDurations` 若未来进入 active contract 的条件

### 9.2 给 Room 2
1. 在下一轮 technical preflight / closeout 中，继续把以下内容保持为技术待决项，不要静默写成 active truth：
   - mixed / auto-routing deeper session contract
   - planner merge / unified planner 收敛方向
   - previewDurations / interval preview 的未来进入条件
2. 继续守住当前 P3.3.2 frozen 边界：
   - 不扩 API schema
...
1. 在下一轮 UI sync 中继续守住：
   - 4 按钮文案不得写成结果事实
   - 首页“背单词”默认继续是 `study_default`，不得被写成 silent reroute
   - active `review_group` continuation 只通过独立 CTA / helper / priority block 承接
   - ReviewPage 继续只表现 `cloud-first + local side-effect`，不得把 local FSRS 或 planner merge 写成用户可依赖事实
2. 若 Room 1 下一轮 focus 选择更深 review planning contract，则仍需单开新 round pin，不得在 UI 同步中静默升格 mixed / auto-routing / unified planner

### 9.4 给 Room 4
1. 把新增的 BR-033 / BR-034 纳入最小回归集
2. 测试至少覆盖：
   - 首页“背单词”入口继续默认直达 `StudyPage`
   - active `review_group` continuation 只能通过独立 CTA / helper / priority block 承接，不得 silent reroute
   - ReviewPage 仍以 `review_group` 作为云端 truth layer
   - local FSRS 继续只作为 device-side scheduling / side-effect owner，不被实现层误写成 unified planner 或主复习计划已更新
   - mixed / auto-routing / unified planner 继续保持 pending，不得被静默实现
...
- **This file:** `BR-OPP-001_v0.2.4.md`
- **Owner:** Room 3
- **Suggested next state after review:** active after Room 1 runtime-baseline update（当前运行态 active BR baseline 仍为 `BR-OPP-001_v0.2.2.md`，待 Main / STATUS 更新）
- **Runtime note:** 本稿以上一版 candidate BR `BR-OPP-001_v0.2.3.md` 为 base，并吸收 P3.3.2 已收口的 `session_entry_policy_v1` / `planner_owner_split_v1` 决策；**current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **P3.3.2 note:** 本版继续在 P3.3 / P3.3.1 基础上，吸收 P3.3.2 已收口的规则：`session_entry_policy_v1`、`planner_owner_split_v1`、`home_word_entry = study_default`、active `review_group` continuation 高优先但不等于 silent reroute、以及 ReviewPage 的 `cloud-first + local side-effect` 最小合同；mixed / auto-routing / unified planner / 统一学习页 / 完整 SRS / `previewDurations` future re-entry 继续保持 pending。
- **Maintenance rule:**
  1. 未来凡涉及 4 按钮业务语义、session entry / auto-routing、review planner owner / planner merge、preview / schedule explanation 的结论变更，必须更新本文件
