...
**Base merge:** `BR-OPP-001_v0.2.11.md` + `P3.3.10 closeout inputs`  
...
- `BR-OPP-001_v0.2.11.md`（latest BR review basis / patch base for this round）
...
> Note:
> 1. 本稿以上述 current review basis 为准，并以 `P3.3.10 — Fuller Cutover / review_group Exit-Gate / DB-API Uplift Judgment Round` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.11.md` 的 full BR 结构上，增量吸收 P3.3.10 已收口、且已被 Room 1 同意进入主 BR 回写的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.10 规则；fuller cutover / exit-gate / uplift judgment 的结果仍不得被写成 runtime owner shift completed、ReviewPage local-serving full runtime cutover、`review_group` 真退场、active DB/API baseline uplift absorbed、cleanup / old-path purge、auto-routing runtime、unified planner / planner merge、DB schema rewrite、API core semantics rewrite 等既成事实。
...
- **This file:** `BR-OPP-001_v0.2.12.md`
- **Owner:** Room 3
...
- **Runtime note:** 本稿以 `BR-OPP-001_v0.2.11.md` 为 base，并吸收 P3.3.10 已收口的 `fuller_cutover_rule_set_v1`、`review_group_exit_gate_v2`、`cutover_vs_fact_owner_boundary_v2`、`db_api_uplift_judgment_v1`、`retained_anchor_to_exit_transition_v1` 与 `phase4_writeback_order_v1` 的最小合同；**current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **P3.3.10 note:** 本版继续在 P3.3 / P3.3.1 / P3.3.2 / P3.3.3 / P3.3.4 / P3.3.5 / P3.3.6 / P3.3.7 / P3.3.8 / P3.3.9 基础上，吸收 P3.3.10 已收口的规则：fuller cutover 当前只允许从 ReviewPage non-continuation seam 扩大到 continuity-adjacent、仍不碰首页 route / active continuation / final fact owner 的 very narrow next subset；`review_group` 当前继续保持 current owner + retained anchor + compatibility anchor + deprecated candidate，且只进入 exit-gate judgment-ready 条件层；DB/API 当前只进入 uplift-judgment-ready seam families；active DB/API baseline uplift、`review_group` 真退场、full cutover completed、cleanup / old-path purge、以及任何用户可见 cutover 宣告继续保持 pending。
- **Maintenance rule:**
