...
**Base merge:** `BR-OPP-001_v0.2.7.md` + `P3.3.6 closeout inputs`  
**Merge policy:** 保留 `v0.2.7` 的 full BR 结构，并将 P3.3.6 已收口的规则吸收为单文件可读的增量主文档候选
...
- `BR-OPP-001_v0.2.7.md`（latest BR review basis / patch base for this round）
...
> Note:
> 1. 本稿以上述 current review basis 为准，并以 `P3.3.6 — Local-Serving Compatibility Contract / Shadow-Mode Entry Round` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.7.md` 的 full BR 结构上，增量吸收 P3.3.6 已收口、且已被 Room 1 同意进入主 BR 回写的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.6 规则；runtime owner shift completed、ReviewPage local-serving runtime cutover、local due queue 接管当前页面真相、`review_group` 退出运行态、auto-routing runtime、unified planner / planner merge、DB / API core rewrite 等继续保持 pending。
...
### 9.1 给 Room 1
1. 将本文件作为 `BR-OPP-001_v0.2.5.md` 的增量升版候选，供下一轮治理层 SSOT 吸收 / pin
2. 在 Main / Status 中吸收以下新增已冻结结论：
   - `review_readiness_policy_v1`：页面级 readiness truth = cloud aggregate / review-serving layer；local FSRS = scheduling candidate input
   - `review_priority_policy_v1`：最小 hierarchy = continuation > cloud-confirmed due review > cloud-confirmed high-priority review > new words > session
   - `review_group_generation_policy_v1`：generation owner 继续在 cloud review-serving layer；active group completion 是 next-group 前提之一；`next_group_eligible` ≠ `next_group_generated`
   - `schedule_source_contract_v1`：local FSRS = scheduling candidate signals；cloud `review_group` = serving outputs；owner split 不等于 planner merge
   - `previewDurations` 在 P3.3.3 当前轮继续 deferred，不进入 active contract / 稳定 UI 事实
3. 继续保持 pending / future-focus item 不被偷升格：
   - full priority scoring / exact group size / full generation algorithm
   - mixed / auto-routing runtime contract
   - unified planner / planner merge
   - stronger ReviewPage bridge contract
   - `previewDurations` future re-entry 条件

### 9.2 给 Room 2
1. 在下一轮 technical preflight / closeout 中，继续把以下内容保持为技术待决项，不要静默写成 active truth：
   - readiness 的完整 reason enum / threshold algorithm / local-only mode
   - full priority scoring / exact group size / full generation algorithm
   - planner merge / unified planner 收敛方向
   - `previewDurations` / interval preview 的 future re-entry 条件
2. 继续守住当前 P3.3.3 frozen 边界：
   - 不把 local FSRS 写成页面级 readiness truth
   - 不把 local candidate 写成 priority winner 或 group producer
   - 不把 owner split 静默升级为 planner merge
   - 不把 `previewDurations` 拉回 contract / UI / assertion

### 9.3 给 Room 5
1. 在下一轮 UI sync 中继续守住：
   - readiness / priority / generation 的页面承接只表达最小合同层，不表达完整算法
   - continuation 高优先不等于 silent reroute
   - local FSRS 不得被写成 readiness truth、priority winner、group producer 或 unified planner
   - `previewDurations` 继续 deferred，不进入首页 / StudyPage / ReviewPage / helper / explanation
2. 若 Room 1 下一轮选择更深 review planning contract，仍需单开新 round pin，不得在 UI 同步中静默升格 exact group size、preview explanation、mixed / auto-routing 或 unified planner

### 9.4 给 Room 4
1. 把新增的 BR-035 ~ BR-039 纳入最小回归集
2. 测试至少覆盖：
   - `ready_now` / `not_ready_now` / `next_group_eligible` / `temporarily_unservable` 的最小正确表达
   - continuation > due / high-priority / new words / session 的 hierarchy-only 逻辑
   - active group completion gating 与 `next_group_eligible` ≠ `next_group_generated`
   - local FSRS 继续只作为 scheduling candidate / device-side owner，不被误写成 unified planner 或页面 readiness truth
   - `previewDurations` 当前不显示、不作为通过标准、不被写成 schedule fact
...
- **This file:** `BR-OPP-001_v0.2.8.md`
- **Owner:** Room 3
...
- **Runtime note:** 本稿以上一版 candidate BR `BR-OPP-001_v0.2.7.md` 为 base，并吸收 P3.3.6 已收口的 `local_serving_candidate_contract_v1`、`review_group_compatibility_posture_v1`、`fact_settlement_ingest_contract_candidate_v1`、`session_entry_and_routing_compat_v1`、`deprecation_markers_and_writeback_plan_v1` 与 `shadow_parity_test_strategy_v1` 的最小合同；**current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **P3.3.6 note:** 本版继续在 P3.3 / P3.3.1 / P3.3.2 / P3.3.3 / P3.3.4 / P3.3.5 基础上，吸收 P3.3.6 已收口的规则：local-serving 只进入 candidate / compatibility / shadow parity 层；`review_group` 继续保持 current runtime serving owner，同时进入 compatibility anchor + deprecated candidate；fact / settlement truth 继续保持后端收口，local-serving evidence 只进入 ingest candidate；首页 runtime 继续保持 `study_default` + no silent reroute；write-back 与 shadow/parity 测试分层被正式写硬；runtime owner shift completed、ReviewPage local-serving cutover、`review_group` 退出运行态、DB/API core rewrite、auto-routing runtime、unified planner / planner merge 与 full sync 等继续保持 pending。
- **Maintenance rule:**
