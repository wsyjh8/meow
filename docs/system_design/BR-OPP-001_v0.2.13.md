...
**Base merge:** `BR-OPP-001_v0.2.12.md` + `P3.3.11 closeout inputs`  
**Merge policy:** 保留 `v0.2.11` 的 full BR 结构，并将 P3.3.10 已收口的规则吸收为单文件可读的增量主文档候选
...
- `BR-OPP-001_v0.2.12.md`（latest BR review basis / patch base for this round）
...
> Note:
> 1. 本稿以上述 current review basis 为准，并以 `P3.3.11 — Fuller-Cutover Execution / review_group Exit-Candidate / DB-API Uplift-Readiness Round` 的推进层事实作为吸收边界。
> 2. 本稿目标不是重写整份 BR，而是在 `BR-OPP-001_v0.2.12.md` 的 full BR 结构上，增量吸收 P3.3.11 已收口、且已被 Room 1 同意进入主 BR 回写的规则。
> 3. 本稿只吸收 **已能跨 BR / UI / DB / API / TEST 稳定引用** 的 P3.3.11 规则；execution-ready subset / exit-candidate / uplift-readiness 的结果仍不得被写成 runtime owner shift completed、ReviewPage local-serving full runtime cutover、`review_group` true exit、active DB/API baseline uplift absorbed、cleanup / old-path purge、auto-routing runtime、unified planner / planner merge、DB schema rewrite、API core semantics rewrite 等既成事实。
...
- **This file:** `BR-OPP-001_v0.2.13.md`
- **Owner:** Room 3
...
- **Runtime note:** 本稿以 `BR-OPP-001_v0.2.12.md` 为 base，并吸收 P3.3.11 已收口的 `fuller_cutover_execution_rule_set_v1`、`review_group_exit_candidate_v1`、`db_api_uplift_readiness_v1`、`cutover_vs_fact_owner_boundary_v3`、`retained_anchor_narrowing_guardrail_v1` 与 `phase5_writeback_order_v1` 的最小合同；**current runtime active baseline remains the version pinned by Main / STATUS until Room 1 updates active versions**.
- **P3.3.11 note:** 本版继续在 P3.3 / P3.3.1 / P3.3.2 / P3.3.3 / P3.3.4 / P3.3.5 / P3.3.6 / P3.3.7 / P3.3.8 / P3.3.9 / P3.3.10 基础上，吸收 P3.3.11 已收口的规则：fuller-cutover 当前允许前进一步到 ReviewPage continuity-adjacent serving-adapter family 与首页 review 承接层的 retained-anchor-aware execution-ready subset；`review_group` 当前只进入 exit-candidate，继续保持 current runtime owner + retained fallback anchor + compatibility anchor + deprecated candidate；DB/API 当前只进入 uplift-readiness seam families；validated stronger-ingest candidate 当前仍不越过 backend final fact owner 边界；full cutover completed、`review_group` true exit、active DB/API baseline uplift absorbed、cleanup / old-path purge、homepage route switch、active continuation source switch、auto-routing runtime、final fact owner shift 与任何用户可见 cutover / exit / uplift absorbed 宣告继续保持 pending。
- **Maintenance rule:**
